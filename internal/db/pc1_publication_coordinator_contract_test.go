...
// pc1CoordinatorMethodAllowlist is the inventoried method set of
// PublicationCoordinator. PC-1 grants exactly one capability, the pure
// settlement rule. A Publish/Stage/Repair/Head/Settle method is a later PC's
// deliberate step and must be added here (and characterized) when it lands.
var pc1CoordinatorMethodAllowlist = []string{"SettlementFor"}
...
// pc1ForbiddenPublicationImports are import paths the skeleton must never
// take: process-local synchronization (a mutex is not multi-DC authority),
// the driver, and the storage/API layers (no I/O, no cycles).
var pc1ForbiddenPublicationImports = []string{
	"sync",
	"sync/atomic",
...
func TestPC1PublicationPackageIsStatelessAndStorageFree(t *testing.T) {
	...
	pc1WalkProductionFiles(t, func(relPath string, file *ast.File) {
		files++
		for _, path := range pc1ImportPaths(file) {
			for _, forbidden := range pc1ForbiddenPublicationImports {
				if path == forbidden || strings.HasPrefix(path, forbidden+"/") {
					violations = append(violations, relPath+" imports "+path)
				}
			}
			first := path
			...
			if strings.Contains(first, ".") {
				violations = append(violations, relPath+" imports non-stdlib "+path)
			}
		}
		for _, decl := range file.Decls {
			gen, ok := decl.(*ast.GenDecl)
...
func TestPC1PublicationCoordinatorMethodSetIsInventoried(t *testing.T) {
	...
	want := append([]string{}, pc1CoordinatorMethodAllowlist...)
	sort.Strings(want)
