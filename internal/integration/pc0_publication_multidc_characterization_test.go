...
func TestPC0PublicationMultiDCCharacterization(t *testing.T) {
	...
	pc0PublicationMatrix.record(t, "M9", "PRIOR-EVIDENCE-NOT-RERUN, resolved 2026-09-11: audit 2026-09-10 (scripts/pc0-initial-head-xdc-probe.sh bug mode, real 3-DC) showed the pre-fix unconditional initializer shape (createInitialCommit/InitializeLibraryFS) executed from a blind dc-eu reverting an LWT-published HEAD in every DC; 2026-09-11 (scripts/h1-initial-head-multidc-validation.sh, real 3-DC, handler-level) both production initializers driven from blind dc-eu keep and return the HEAD dc-na published through FSHelper.InitializeLibraryHeadIfUnset (IF head_commit_id = null AND created_at != null); ISSUE-LIBRARY-INITIAL-HEAD-CONCURRENCY-01 resolved")
