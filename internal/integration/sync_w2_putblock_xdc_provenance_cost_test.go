...
// cleanupW2SyncXDCProvenanceFixture removes exactly the rows one
...
// can't drift from what the writer used. Shared by both the single-block
// correctness fixture and the per-N cost fixtures, so every
// AddProvisionalBlockReferenceWithExpiry call this package makes is retired
// through the same three-surface path.
//
...
//
// A cleanup failure fails the test (t.Errorf), not just t.Logf: silently
...
// t.Errorf rather than t.Fatalf is load-bearing here, not a style choice:
// this runs inside a loop over up to 1111 blocks in a single t.Cleanup, and
// Fatalf would abort that closure via runtime.Goexit on the first failure,
// silently skipping cleanup for every remaining block. Errorf marks the run
// red and keeps going, so every surface for every block is still attempted.
func cleanupW2SyncXDCProvenanceFixture(t *testing.T, database *dbpkg.DB, consistency gocql.Consistency, orgID, blockID, referrer string, expiresAt time.Time) {
	t.Helper()
	if err := database.Session().Query(`DELETE FROM block_references WHERE org_id = ? AND block_id = ? AND referrer = ?`,
		...
		t.Errorf("cleanup: delete block_references for org=%s block=%s failed: %v", orgID, blockID, err)
	}
	if err := database.Session().Query(`DELETE FROM gc_provisional_block_refs WHERE org_id = ? AND block_id = ? AND referrer = ?`,
		orgID, blockID, referrer).Consistency(consistency).Exec(); err != nil {
		t.Errorf("cleanup: delete gc_provisional_block_refs for org=%s block=%s failed: %v", orgID, blockID, err)
	}
	...
	if err := batch.Exec(); err != nil {
		t.Errorf("cleanup: delete gc_provisional_block_refs_by_day for org=%s block=%s failed: %v", orgID, blockID, err)
	}
}
	...
	runScenario := func(t *testing.T, n, hitCount int) time.Duration {
		...
		for i := range blockIDs {
			...
			if i < hitCount {
				...
				t.Cleanup(func() { cleanupW2SyncXDCProvenanceFixture(t, database, gocql.LocalQuorum, orgID, blockID, referrer, expiresAt) })
			}
		}
