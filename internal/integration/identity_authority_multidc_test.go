...
// local Paxos round and three "authoritative" digests would coexist. That is
// the outcome this leg must make impossible.
...
func TestIdentityAuthorityConcurrentCrossDCClaimsHaveOneWinner3DC(t *testing.T) {
	...
	for round := 0; round < rounds; round++ {
		...
		for dc := range sessions {
			...
			digests[dc] = dbpkg.FileIdentityDigest(library, fsID, "file", 10, logical, canonical)
		}
		...
		for dc, database := range sessions {
			...
			go func(dc string, database *dbpkg.DB) {
				defer wg.Done()
				<-start
				res, err := dbpkg.ClaimIdentityAuthority(ctx, database.Session(), library, dbpkg.IdentityKindFile, fsID, dbpkg.SupportedIdentityDigestVersion, digests[dc])
				mu.Lock()
		...
		if established != 1 {
			t.Fatalf("round %d: established winners=%d across three DCs, want exactly 1; the claim is not in one global SERIAL domain", round, established)
		}
		...
		if matches != 1 {
			t.Fatalf("round %d: winning digest matches %d contenders, want 1", round, matches)
		}
		...
		for dc, database := range sessions {
			...
			res, err := dbpkg.ClaimIdentityAuthority(ctx, database.Session(), library, dbpkg.IdentityKindFile, fsID, dbpkg.SupportedIdentityDigestVersion, winner)
...
func TestIdentityAuthorityClaimSurvivesCrossDCDeleteAndRecreate3DC(t *testing.T) {
	...
	fsID := "f-" + uuid.NewString()
	logical := []string{"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"}
	digestA := dbpkg.FileIdentityDigest(library, fsID, "file", 10, logical, []string{strings.Repeat("a", 64)})
	digestB := dbpkg.FileIdentityDigest(library, fsID, "file", 10, logical, []string{strings.Repeat("b", 64)})

	if res, err := dbpkg.ClaimIdentityAuthority(ctx, na.Session(), library, dbpkg.IdentityKindFile, fsID, dbpkg.SupportedIdentityDigestVersion, digestA); err != nil || res.Outcome != dbpkg.IdentityClaimEstablished {
	...
	for dc, database := range map[string]*dbpkg.DB{"dc-na": na, "dc-asia": asia} {
		...
		outcome, err := dbpkg.VerifyIdentityAuthority(ctx, database.Session(), library, dbpkg.IdentityKindFile, fsID, dbpkg.SupportedIdentityDigestVersion, digestA)
		if err != nil || outcome != dbpkg.IdentityClaimIdempotent {
			t.Fatalf("%s still verifies the original claim: outcome=%v err=%v, want idempotent", dc, outcome, err)
		}
	}
}
