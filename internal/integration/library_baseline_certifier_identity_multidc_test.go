...
func TestLibraryBaselineCertifierIdentityDivergence3DC(t *testing.T) {
	...
	fsRoot, fsFile := stableID("fs-root"), stableID("fs-file")
	commitRootA, commitRootB := stableID("commit-root-a"), stableID("commit-root-b")
	commitFileA, commitFileB := stableID("commit-file-a"), stableID("commit-file-b")
...
func TestLibraryBaselineCertifierIdentityAuthorityUnavailable3DC(t *testing.T) {
	...
	head, rootFSID, fileFSID := "pc-d1b1-identity-unavailable-head-"+strings.ReplaceAll(runID, "-", ""), stableID("root"), stableID("file")
