      ...
      <React.Fragment>
        ...
        {!this.props.isCustomPermission && (
          <div className="detail-btn btn-group">
            <button className="btn btn-secondary btn-icon ml-1 sf2-icon-info" id='detail' title={gettext('Properties')} aria-label={gettext('Properties')} onClick={this.switchViewMode}></button>
          </div>
        )}
      </React.Fragment>
    );
