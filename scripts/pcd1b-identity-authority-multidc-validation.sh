#!/usr/bin/env bash
# Real 3-DC evidence for the PC-D1B identity-authority primitive. Resources are
# private to this fixture; it never attaches to or stops the default stack.
set -euo pipefail

cd "$(dirname "${BASH_SOURCE[0]}")/.."

PROJECT=sesamefs-pcd1b-cassandra
PREFIX=sesamefs-pcd1b-cassandra
RUNNER=sesamefs-pcd1b-integration-runner
BACKEND=sesamefs-pcd1b-backend
MINIO=sesamefs-pcd1b-minio
IMAGE=sesamefs-pcd1b-gotest
BACKEND_IMAGE=sesamefs-pcd1b-backend-image
NETWORK=
KEEP=0
...
cleanup() {
	...
	if [ "$KEEP" -eq 0 ]; then
		...
		echo "PC-D1B 3-DC fixture left running (--keep)"
	fi
	exit "$rc"
}
trap cleanup EXIT
...
docker run -d --name "$BACKEND" --network "$NETWORK" \
	...
	-e ONLYOFFICE_ENABLED=false -e ONLYOFFICE_JWT_SECRET=pcd1b-test-jwt \
	-e SHARE_LINK_HMAC_KEY=pcd1b-test-share-key -e ACCOUNTS_DISABLE_ORG_USER_WRITES=true \
...
step "Run PC-D1B identity-authority evidence with LOCAL_SERIAL sessions"
docker exec "$RUNNER" env \
	...
	SESAMEFS_REQUIRE_IDENTITY_AUTHORITY_EVIDENCE=1 \
