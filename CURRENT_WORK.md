...
Three independent audit rounds hardened this slice. Round 2: the fan-out calling the scope gate had no cancellation (`syncCommitProvenancedBlockIDs` now uses `errgroup.WithContext`, bounding a degraded datacenter's blast radius to one concurrency wave, 20, instead of O(N)); the real 3-DC all-cross-DC-hit cost scenario was missing from the characterization. Round 3: the consistency guard pinned the named constant's *value* but not that `BlockReferenceExistsEachQuorum` actually *binds* it (`TestBlockReferenceExistsEachQuorumBindsTheNamedConsistencyConstant` + mutation M15 close that); the cost benchmark reused overlapping block slices across N, letting Cassandra's own read-repair silently heal a shared block's local visibility between scenarios (fixed with disjoint per-N block sets and an explicit locally-blind precondition check before each timed measurement -- this corrected the real 3-DC N=1000 number from 3.4-5.0s down to 1.67s); cost-test cleanup was best-effort (`t.Logf`) instead of fail-closed (`t.Fatalf`) for both the single-DC and 3-DC fixtures; and the scope-gate-only cost numbers were being read as if they were the full end-to-end HEAD-readiness cost for a recovered block, which they are not (the rest of #206's pipeline -- `ProbeBlockReuse`, renewal, exact-P validation -- still runs for a recovered block, though all of it stays `LOCAL_QUORUM`/session-inherited, not newly WAN-bound).
...
Real 3-DC evidence: RED without the fallback, GREEN with it, one-DC-down fails closed, and the corrected all-cross-DC-hit cost measurement at N=1/10/100/1000. 15/15 mutations (M1-M15) produce the expected RED. Does not touch expired provenance, GC, G1-G5, or X1; `GC_ENABLED=false`.
