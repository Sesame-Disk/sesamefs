...
**Session**: W2 Sync PutBlock cross-DC provenance visibility (PR #210), branch `fix/w2-sync-putblock-xdc-provenance`, following up #206 (merged). Closes `ISSUE-SYNC-PUTBLOCK-CROSS-DC-PROVENANCE-VISIBILITY-01`: the pre-HEAD scope gate (`syncBlockHasOwnLivenessProvenanceFn`) now falls back to an exact-referrer `EACH_QUORUM` read (`BlockReferenceExistsEachQuorum`, the new analog of the existing `BlockHasReferencesGlobal`) only on a clean local miss, so a PutBlock acknowledged in one datacenter is not misclassified as absent when HEAD runs from another before normal replication converges. The fast path (local hit or local error) pays zero added WAN. A second-round audit found the fan-out that calls the scope gate had no cancellation, so a degraded datacenter could turn into O(N) failing lookups for a large commit; `syncCommitProvenancedBlockIDs` now uses `errgroup.WithContext` so the first fatal error stops scheduling new work, bounding the blast radius to roughly one concurrency wave (20). The audit also asked whether genuinely-unprovenanced (dedup-only) blocks now depend on every datacenter's availability -- yes, and `docs/KNOWN_ISSUES.md` has the explicit analysis of why that is the unavoidable consequence of using the only signal available (a `LOCAL_QUORUM` miss) without weakening correctness (a plain `QUORUM` fallback is not safe with one replica per DC) or expanding scope (no client hints, no write-consistency change), plus the decision to accept it with the blast radius bounded. Real 3-DC evidence proves RED without the fallback and GREEN with it, a one-DC-down leg proving fail-closed behavior, and a real cross-DC all-hit cost measurement at N=1/10/100/1000 (the single-DC characterization structurally can't produce that scenario). `TestSyncBlockReferenceCrossDCFallbackConsistencyIsEachQuorum` pins the fallback's consistency by value; 14/14 mutations (M1-M14) produce the expected RED. Does not touch expired provenance, GC, G1-G5, or X1; `GC_ENABLED=false`.
