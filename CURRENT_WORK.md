...
**W2 Sync `PutBlock` -> HEAD slice (2026-09-07, branch `fix/w2-sync-putblock-head-continuity`, based on `main` merge `57fd090d2e98013dd875a4d5fe6c4b95c8df5f08`, PR #205's tip):** closes the two `CONDITIONAL` Sync rows of `docs/R3-LIVENESS-CONTINUITY.md` ("Sync `PutBlock` followed by HEAD", "Sync retry from another pod") through pre-HEAD, reusing #205's `published_block_reference_repairs` schema and exported `internal/api/v2/publish_repair.go` helpers with no new migration. Before the HEAD CAS, in both `handleSyncHeadPromotion` and `tryAutoMergeSyncHeadPromotion`: (1) added files' block IDs are resolved to canonical form once per commit and split back per file (`resolveSyncCommitAddedFilesCanonical`) so the durable repair row never carries a flattened cross-file aggregate or an external SHA-1; (2) a durable repair row is queued per added file; (3) own liveness is renewed and exact physical placement is fail-closed re-validated (`ensureSyncCommitBlockPublicationReadiness`), scoped strictly to blocks that already have a real `up:sync:<repo>:<block>` reference (`syncBlockHasOwnLivenessProvenanceFn`) — a block with no such reference (client-side dedup, cross-commit reuse) is left untouched, so "Sync commit whose block had no associated PutBlock" is not reclassified. Post-HEAD: `repairPublishedSyncCommitBlockDelta` renews liveness best-effort (never fail-closed — an already-reachable commit cannot be rejected retroactively) and settles the repair row through the same `finalizeSyncCommitBlockDeltaAndSettleRepairIntent` wrapper used by the success/failure paths (clear on success, schedule for retry on failure). CAS-conflict handling: a genuinely divergent loss (current HEAD moved to neither the target nor the expected parent) synchronously clears its own repair-row entry; an idempotent same-target "loss" to another writer never does, since that writer's own crash recovery may still need it — auto-merge's `mergedCommitID` is always attempt-unique so this ambiguity never arises there. All new reads/writes are LOCAL_QUORUM (`ProbeBlockReuse`, `ValidateBorrowedFSPublicationAuthority`, `AddProvisionalBlockReferenceWithExpiry`, `BlockReferenceExists`), never SERIAL/EACH_QUORUM, and the existing `internal/db/r3_publication_*` hot-path guards pass unmodified because the new logic is delegated through already-reviewed helper functions rather than inlined `h.db.*` calls in the guarded root bodies. Unit coverage (`internal/api/sync_w2_putblock_head_test.go`) proves ordering, scope-gating, fail-closed placement validation, repair-row rollback/settlement, and CAS-loser identity at the seam level. Real-Cassandra/MinIO integration evidence (`internal/integration/sync_w2_putblock_head_integration_test.go`, gate `SESAMEFS_REQUIRE_W2_SYNC_PUTBLOCK_HEAD_EVIDENCE=1`, wired into `TestMain` and `go-integration-test`/`go-all-test`) covers five named legs — normal-flow renewal, cross-node idempotent repair without process memory, active-GC-claim fail-closed rejection, definitive-CAS-loser cleanup, and auto-merge with real blocks — all green, plus the full canonical integration suite with zero regressions. Ten deliberate mutations (`scripts/w2-sync-putblock-head-mutation-validation.sh`) all produced the expected RED. Known accepted residual risk (not a new regression, extends #205's own accepted tradeoff — see `ISSUE-PUBLISH-REPAIR-REACHABILITY-01`/`ISSUE-PUBLISH-REPAIR-TIMEOUT-CLEANUP-01`): a Sync direct-HEAD repair row that is never provably reachable nor provably a definitive loser (crash mid-flight, or a same-target race whose outcome this process never learns) is retained forever by the existing repair sweep classifier, which only ever promotes or retains — never deletes on inference. Does not touch G1, `gc_s3_orphans`, PREPARED/COMMITTED, or any GC schema. `GC_ENABLED=false`.
