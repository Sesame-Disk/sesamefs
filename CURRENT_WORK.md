...
**Session**: W2 Sync PutBlock cross-DC provenance visibility, branch `fix/w2-sync-putblock-xdc-provenance`, following up #206 (merged). Closes `ISSUE-SYNC-PUTBLOCK-CROSS-DC-PROVENANCE-VISIBILITY-01`: the pre-HEAD scope gate (`syncBlockHasOwnLivenessProvenanceFn`) now falls back to an exact-referrer `EACH_QUORUM` read (`BlockReferenceExistsEachQuorum`, the new analog of the existing `BlockHasReferencesGlobal`) only on a clean local miss, so a PutBlock acknowledged in one datacenter is not misclassified as absent when HEAD runs from another before normal replication converges. The fast path (local hit or local error) pays zero added WAN. Real 3-DC evidence proves RED without the fallback and GREEN with it, plus a one-DC-down leg proving the fallback fails closed rather than silently treating unavailability as absence. Cost/availability characterization is documented in `docs/KNOWN_ISSUES.md` with an explicit single-DC-measurement caveat; concurrency tuning against real multi-region latency is deferred to `ISSUE-SYNC-PUTBLOCK-READINESS-HOTPATH-COST-01`. Does not touch expired provenance, GC, G1-G5, or X1; `GC_ENABLED=false`.
