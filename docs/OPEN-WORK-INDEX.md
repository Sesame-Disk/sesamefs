...
| `ISSUE-PUBLISH-REPAIR-RENEWAL-AFTER-CLASSIFY-01` | HIGH (P1) ✅ **Resolved 2026-09-14** — PRE-X1 / PRE-GC | Closes the classifier-induced gap: once the pre-classify renewal of `pub:<repo:commit:fsID>` (existing `StillPending → Add → StillPending` helper, compensation on gone or on partial fan-out failure) completes, the bounded walk cannot expire the pin; renewal failure fails closed without walking; one renewal per visit; a write-ahead cleanup intent (`published_repair_liveness_cleanups`, migration 025) is written before every pin and processed by the sweep, so a row cleared underneath the walk or underneath a failed positive settlement has its pin removed by the same visit or, after any failure or process loss, by the sweep — no ownerless pin added versus `main`; REACHABLE keeps `renew → classify → promote → remove pub: → delete intent → delete row`. Unit + M1–M15 mutations (46/46 RED) + real-Cassandra `renewal_before_classify` leg (external clear during walk, UNKNOWN, REACHABLE, durable rediscovery by the sweep). Not closed: discovery-after-expiry and expiry during the sequential per-block fan-out (`ISSUE-PUBLISH-REPAIR-DISCOVERY-SCALE-01`, `ISSUE-GC-PUB-REF-ZERO-REF-01`), the pre-existing gone-read→DELETE requeue race, R31/W2/GC | [known issue](./KNOWN_ISSUES.md#issue-publish-repair-renewal-after-classify-01) |
