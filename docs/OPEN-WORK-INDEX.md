...
| `ISSUE-PUBLISH-REPAIR-REACHABILITY-CONVERGENCE-01` | HIGH (P1) ✅ **Resolved 2026-09-12** — PRE-X1 / R31 convergence | Resumable SERIAL-anchored walk with durable next-unread cursor; later HEAD movement does not restart in-flight work; clean genesis persists exhaustion before re-observing HEAD; missing rows are no-ops; UNKNOWN retains; a live row renews `pub:<repo:commit:fsID>:<producer_token>` before classify, and each producer's cleanup/fence is token-isolated. Sync success does not pay per-block repair-owned DELETE. | [known issue](./KNOWN_ISSUES.md#issue-publish-repair-reachability-convergence-01) |
| `ISSUE-PUBLISH-REPAIR-OWNED-PUB-CLEANUP-RACE-01` | MEDIUM (P2) ✅ **Resolved 2026-09-15** | The repair-owned physical referrer is now `pub:<repo:commit:fsID>:<producer_token>`; concurrent visits and requeues own distinct witnesses, so an old fence cannot remove a newer producer's pin even with equal or inverted leases. CONSUMED witnesses reach their 35-day terminal delete independently. Ordinary Sync still does not pay per-block repair-owned DELETEs | [known issue](./KNOWN_ISSUES.md#issue-publish-repair-owned-pub-cleanup-race-01) |
| `ISSUE-PUBLISH-REPAIR-RENEWAL-AFTER-CLASSIFY-01` | HIGH (P1) ✅ **Resolved 2026-09-14** — PRE-X1 / PRE-GC | Pre-classify renewal is durable and producer-token keyed: `pub:<repo:commit:fsID>:<producer_token>` is written before the bounded walk, every pin and fence uses the producer lease timestamp, and each CONSUMED producer has an independently bounded 35-day terminal lifecycle. CONSUMED re-fences and reaches the 35-day terminal delete without waiting for the identity to be gone; equal/inverted leases cannot cross-fence a requeue. Unit M1–M40 (72/72 expected RED), including M39/M40 pending-partial retention, plus real Cassandra renewal/rediscovery and real 3-DC stale-lease and producer-isolation evidence cover the safety contract. Pending identities retain all finished witnesses; pending witness storage may grow until safe reuse is backed by durable full-coverage proof, so bounded compaction remains a follow-up. Discovery-after-expiry and sequential fan-out expiry remain separate R31 follow-ups | [known issue](./KNOWN_ISSUES.md#issue-publish-repair-renewal-after-classify-01) |
...
| `ISSUE-BLOCK-REFERENCES-GC-GRACE-CERTIFICATION-01` | MEDIUM (P2) - PRE-GC follow-up, not a #220 blocker | The repair cleanup re-fence interval (3d) is certified against `gc_grace_seconds = 864000` pinned by migration 025 and a Go guard, not against the live `system_schema.tables`; an operator lowering it later silently shortens the fence (over-retention only). Intended fix: readiness gate reading the table property | [known issue](./KNOWN_ISSUES.md#issue-block-references-gc-grace-certification-01) |
| `ISSUE-BLOCK-REFERENCES-RF-TOMBSTONE-CERTIFICATION-01` | MEDIUM (P2) - PRE-GC follow-up, not a #220 blocker | The repair cleanup re-fence (`EACH_QUORUM` tombstones every 3d for the 35d pin TTL) is certified for one replica per DC, the topology the 3-DC evidence runs; with RF>1 a replica that missed a fence round can resurrect a retired producer's `pub:` after the others purge their tombstone (over-retention only). Before destructive GC on RF>1: repair inside `gc_grace_seconds`, `only_purge_repaired_tombstones`, or equivalent | [known issue](./KNOWN_ISSUES.md#issue-block-references-rf-tombstone-certification-01) |
