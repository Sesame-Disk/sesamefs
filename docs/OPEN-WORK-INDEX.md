...
**Last updated:** 2026-09-24 (PC-D1B.4 in-flight authority audit)
**PC-D1B.4 status (2026-09-24):** **NO MERGE** until CW-M33's exact paused pre-commit HEAD Paxos race is characterized on Cassandra 5.0.9. CW-M31 remains closed for cross-DC visibility only; absence capability also requires settlement/stability against pre-existing HEAD proposals. CW-M34 freezes the in-flight writer lifetime property and has model plus 3-DC Cassandra characterization; runtime protection/recovery remains PC-D1B.5. CW-M27..CW-M32 stay closed except the explicitly distinct CW-M33/M34 lifetime gaps. Canonical contract and sequencing are in [PC-D1B-CERTIFICATION-WINDOW-FENCE.md](./PC-D1B-CERTIFICATION-WINDOW-FENCE.md). PC-D1B.5 is required before GC activation or a productive consumer. No runtime, schema, consumer, mapping-authority or GC change landed here.
...
| `ISSUE-PCD1B4-CERTIFICATION-WINDOW-FENCE-01` | HIGH (P1) — **NO MERGE** pending CW-M33 real-Cassandra paused pre-commit Paxos race; runtime OPEN (PC-D1B.5, PRE-GC / pre-consumer) | Per-library E/P/S fence remains the architecture. CW-M27..CW-M32 stay closed for their original scope; CW-M31 establishes only cross-DC visibility. CW-M33 requires stable absence: settle pre-existing global-SERIAL HEAD proposals then prove EACH_QUORUM absence; model mutation RED, exact Cassandra 5.0.9 pause still required. CW-M34 requires no admitted write at `w <= g` remain able to succeed hidden after GC; model and 3-DC success-but-hidden characterization exist, implementation is PC-D1B.5. Also frozen: `ClockSafetyLease`, UUIDv5 vectors, canonical `created_at` sentinel, whole-row W, persisted durable `identity_at`, no v1 batch amortization, and repeat reaffirmation on UNKNOWN | [known issue](./KNOWN_ISSUES.md#issue-pcd1b4-certification-window-fence-01) · [PC-D1B.4](./PC-D1B-CERTIFICATION-WINDOW-FENCE.md) |
