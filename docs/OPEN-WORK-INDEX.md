...
**Last updated:** 2026-09-19 (publish-repair dead-row cadence and tracking scope)
...
| `ISSUE-PUBLISH-REPAIR-GONE-CHECK-XDC-AUTHORITY-01` | MEDIUM (P2) — OPEN — PRE-X1 / PRE-GC | The post-renewal gone-check of the repair-owned `pub:` treats a `LOCAL_QUORUM` absence of the repair row as proof it is globally gone and removes liveness; destructive absence needs global-enough authority and must fail closed. No under-retention schedule found in current `main` (successful clears run after `fs:` promotion, known-loser clears remove a proven non-live publication, settlement installs `fs:` first, residue reaps have no blocks, requeues stage a fresh attempt pin first); escalate to P1 if that stops holding. The next renewal design must account for this cleanup-authority decision; implementation may be a separate scoped follow-up unless that design depends on, changes, or removes the path | [known issue](./KNOWN_ISSUES.md#issue-publish-repair-gone-check-xdc-authority-01) |
