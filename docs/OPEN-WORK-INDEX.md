...
| `ISSUE-SYNC-PUTBLOCK-CROSS-DC-PROVENANCE-VISIBILITY-01` | HIGH (P1) ✅ Resolved 2026-09-08 | `syncBlockHasOwnLivenessProvenanceFn` now falls back to an exact-referrer `EACH_QUORUM` read on a clean local miss, recovering provenance acknowledged in another DC; real 3-DC RED->GREEN evidence plus a one-DC-down fail-closed leg | [known issue](./KNOWN_ISSUES.md#issue-sync-putblock-cross-dc-provenance-visibility-01) - cost/availability characterization attached; concurrency tuning tracked separately in `ISSUE-SYNC-PUTBLOCK-READINESS-HOTPATH-COST-01` |
