...
productive consumer. #228 can fail closed before mapping authority exists: a
reachable SHA-1-only identity dependent solely on `block_id_mappings` remains
`identity_unproven` with no witness. Deployment is greenfield (`docs/DEPLOY.md`), so historical cutover and pre-authority backfill are non-goals rather than stages. The certification-window fence is mandatory
...
| `ISSUE-PCD1-CERTIFIED-BASELINE-IMPLEMENTATION-01` | HIGH (P1) — OPEN, PC-D1A foundation landed 2026-09-19 | PR #231 now wires the inventoried commits/fs_objects writers and deleters through the identity gateway and freezes the no-bypass boundary. PC-D1B.2 metadata wiring and characterization are complete (PR #231); #228 adds the fail-closed certifier gate, exact-P tree walk, and non-expiring liveness handshake; SHA-1-only mapping-dependent identities stay `identity_unproven` without a witness until mapping authority exists. Add the separate mapping-authority representation before M18/M19 cold-path promotion. Specify the certification-window fence before destructive GC and the first productive consumer; enable that consumer only after the gates and required mapping coverage are complete. Historical backfill is a greenfield non-goal; no productive consumer or PC-2 work has started | [known issue](./KNOWN_ISSUES.md#issue-pcd1-certified-baseline-implementation-01) · [PC-D1](./PC-D1-INHERITED-DEPENDENCY-CONTINUITY.md) |
