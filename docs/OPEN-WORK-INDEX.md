...
X1 closure. W1 BorrowedFS own-liveness through HEAD is implemented and evidence-backed; W2/R31 still covers full writer continuity. PC-0 ([PUBLICATION-PROTOCOL-CHARACTERIZATION.md](./PUBLICATION-PROTOCOL-CHARACTERIZATION.md)) characterizes the current publication protocol and recommends a future multi-DC PublicationCoordinator; it does not close W2/R31. PC-1 (2026-09-11) introduced the coordinator skeleton and the common publication types in `internal/publication` (attempt identity, tri-state HEAD outcome, settlement disposition, opaque evidence boundary) with zero funnels migrated, zero productive importers, and zero runtime/schema/CL/TTL change; it does not execute any publication sequence and does not close W2/R31. PC-D1 (2026-09-12) closes the inherited-dependency architecture decision (`ISSUE-PC0-INHERITED-DEPENDENCY-CONTINUITY-01`): the certified baseline frontier owns continuity, and `WorkSetScopeNewlyLive` is incremental only with a valid durable witness. PC-D1A authority foundation (canonical witness columns, fail-closed validity, HEAD-fenced baseline CAS, and atomic global-SERIAL HEAD+witness authority) landed 2026-09-19. PC-D1B certifier, exact-P and GC-authority liveness, settlement, lifecycle fencing, and first productive consumer remain required before PC-2; no funnel is migrated. PC-D1B (2026-09-20) adds a distinct metadata prerequisite ahead of the certifier
(`ISSUE-PCD1B-METADATA-IDENTITY-AUTHORITY-01`, decided in
[PC-D1B-METADATA-IDENTITY-AUTHORITY.md](./PC-D1B-METADATA-IDENTITY-AUTHORITY.md)).
The order is: authority-only identity primitive (#230); wire every commit/fs_object
writer and deleter, replace the declaration inventory with a real no-bypass
fence, and establish the claim-cost contract (with recoverable `PutCommit.created_at`
before that writer is wired); land #228's fail-closed certifier gate; add a
separate mapping-authority representation, then cold-path M18/M19 promotion for
SHA-1-only identities; specify the certification-window fence before destructive
GC and the first productive consumer; then enable a consumer only after all gates
and required mapping coverage are complete. #228 can fail closed before mapping
authority exists: a reachable SHA-1-only identity dependent solely on
`block_id_mappings` remains `identity_unproven` with no witness. Deployment is
greenfield (`docs/DEPLOY.md`), so historical cutover and pre-authority backfill
are non-goals rather than stages. The certification-window fence is mandatory
before destructive GC and the consumer, but does not gate #228. P4c-orphan exact
...
| `ISSUE-PCD1-CERTIFIED-BASELINE-IMPLEMENTATION-01` | HIGH (P1) — OPEN, PC-D1A foundation landed 2026-09-19 | Remaining PC-D1B work is staged: first wire every inventoried commits/fs_objects writer and deleter through claims, replace the declaration inventory with a real no-bypass fence, and establish the claim-cost contract; make `PutCommit.created_at` stable and recoverable across retries/crashes before wiring it. Then #228 adds the fail-closed certifier gate, exact-P tree walk, and non-expiring liveness handshake; SHA-1-only mapping-dependent identities stay `identity_unproven` without a witness until mapping authority exists. Add the separate mapping-authority representation before M18/M19 cold-path promotion. Specify the certification-window fence before destructive GC and the first productive consumer; enable that consumer only after the gates and required mapping coverage are complete. Historical backfill is a greenfield non-goal; no productive consumer or PC-2 work has started | [known issue](./KNOWN_ISSUES.md#issue-pcd1-certified-baseline-implementation-01) · [PC-D1](./PC-D1-INHERITED-DEPENDENCY-CONTINUITY.md) |
| `ISSUE-PCD1B-METADATA-IDENTITY-AUTHORITY-01` | HIGH (P1) — OPEN; decision and primitive merged, writer/deleter wiring under PR #231 | Baseline certification resolves `commits` / `fs_objects` and SHA-1-only block dependencies (`block_id_mappings`) through ordinary reads, so a complete but unproven or divergent metadata identity can be mistaken for an authoritative one and certified — today's supported writers share no durable identity-authority protocol, so this does not need an old row to occur. A file authority digest that omits the paired canonical SHA-256 `block_ids` accepts two rows with the same `fs_id` and logical list but different physical dependencies. Identity deletes are unfenced; a covered identity removed between the final revalidation and witness settlement would settle a witness that is already false, which on current evidence only the dormant GC cascade can reach, so that fence is PRE-GC / pre-consumer rather than a certifier merge gate. Claims must pin the canonical global `SERIAL` domain, never the configurable session default. PR #231 wires the semantic writers and deleters through one gateway, freezes the exact display-only exception, recovers commit timestamps from claims, rejects divergent source rows, and records the per-operation claim/read/materialization cost. Minimum certifier correctness (refuse the unproven) is separate from coverage: cold-path mapping promotion is its own work item, while historical cutover/backfill are greenfield non-goals | [known issue](./KNOWN_ISSUES.md#issue-pcd1b-metadata-identity-authority-01) · [PC-D1B](./PC-D1B-METADATA-IDENTITY-AUTHORITY.md) |
