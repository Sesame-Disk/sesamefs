...
| `ISSUE-PUBLISH-REPAIR-RENEWAL-AFTER-CLASSIFY-01` | HIGH (P1) ✅ **Resolved 2026-09-14** — PRE-X1 / PRE-GC | Closes the classifier-induced gap: once the pre-classify renewal of `pub:<repo:commit:fsID>` (existing `StillPending → Add → StillPending` helper, compensation on gone or on partial fan-out failure) completes, the bounded walk cannot expire the pin; renewal failure fails closed without walking; one renewal per visit; a write-ahead, producer-token-keyed, TTL-free cleanup intent (`published_repair_liveness_cleanups`, migration 025) is written PREPARING with a lease before every pin, every pin and every cleanup of that producer carry the lease as their Cassandra timestamp (a late write is shadowed by its tombstone), the lease is renewed inside the fan-out by a conditional LWT, the producer ARMs it afterwards by a conditional payload-carrying LWT, and the sweep consumes it only when armed or past its lease, never without payload, compacting finished producers per pending identity; the destructive absence check (visit and sweep) retains on the session read and escalates a local absence to an `EACH_QUORUM` authority read (fail closed), so a row cleared underneath the walk or underneath a failed positive settlement has its pin removed by the same visit or, after any failure or process loss, by the sweep — no ownerless pin added versus `main`, no local absence can remove liveness, no witness consumed while its producer can still write; REACHABLE keeps `renew → classify → promote → remove pub: → delete intent → delete row`. Unit + M1–M27 mutations (58/58 RED) + real-Cassandra `renewal_before_classify` leg (external clear during walk, UNKNOWN, REACHABLE, durable rediscovery by the sweep) + real 3-DC legs (blind-DC sweep keeps the pin; unavailable DC fails closed). Not closed: discovery-after-expiry and expiry during the sequential per-block fan-out (`ISSUE-PUBLISH-REPAIR-DISCOVERY-SCALE-01`, `ISSUE-GC-PUB-REF-ZERO-REF-01`), the pre-existing gone-read→DELETE requeue race, R31/W2/GC | [known issue](./KNOWN_ISSUES.md#issue-publish-repair-renewal-after-classify-01) |
