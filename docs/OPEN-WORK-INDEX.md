...
| `ISSUE-PUBLISH-REPAIR-RENEWAL-AFTER-CLASSIFY-01` | HIGH (P1) ✅ **Resolved 2026-09-14** — PRE-X1 / PRE-GC | Pre-classify renewal is durable and producer-token keyed: `pub:<repo:commit:fsID>:<producer_token>` is written before the bounded walk, every pin and fence uses the producer lease timestamp, and the sweep's Paxos lifecycle is bounded per producer. CONSUMED re-fences and reaches the 35-day terminal delete without waiting for the identity to be gone; equal/inverted leases cannot cross-fence a requeue. Unit M1–M40 (72/72 expected RED), including M39/M40 pending-partial retention, plus real Cassandra renewal/rediscovery and real 3-DC stale-lease and producer-isolation evidence cover the safety contract. Pending identities retain all finished witnesses; bounded pending compaction remains a follow-up. Discovery-after-expiry and sequential fan-out expiry remain separate R31 follow-ups | [known issue](./KNOWN_ISSUES.md#issue-publish-repair-renewal-after-classify-01) |
