...
| `ISSUE-PUBLISH-REPAIR-REACHABILITY-CONVERGENCE-01` | HIGH (P1) ✅ **Resolved 2026-09-12** — PRE-X1 / R31 convergence | Resumable SERIAL-anchored walk with durable next-unread cursor; later HEAD movement does not restart in-flight work; clean genesis is persisted as exhausted before re-observing HEAD and may re-anchor to a later SERIAL HEAD; missing repair row is a no-op; UNKNOWN still retains; repair-owned `pub:<commitID>` is renewed while unresolved; success settlement best-effort removes that identity before the row | [known issue](./KNOWN_ISSUES.md#issue-publish-repair-reachability-convergence-01) |
| `ISSUE-PUBLISH-REPAIR-OWNED-PUB-CLEANUP-RACE-01` | MEDIUM (P2) — follow-up, not an R31-C1 blocker | Concurrent renewal can re-create repair-owned `pub:<commitID>` after settlement removes it and before the repair row is deleted; leftover `pub:` is TTL-bounded over-retention. Ordering alone cannot prove absence; durable settling state would be a separate protocol | [known issue](./KNOWN_ISSUES.md#issue-publish-repair-owned-pub-cleanup-race-01) |
