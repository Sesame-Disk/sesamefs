...
| `ISSUE-SYNC-PUTCOMMIT-NOT-WRITE-ONCE-01` | HIGH (P1) ✅ Fixed by PR #208 (pending merge) | `PutCommit` now arbitrates immutable (parent_id, root_fs_id) with first-writer-wins LWT; identical retries are idempotent and conflicts are rejected | [known issue](./KNOWN_ISSUES.md#issue-sync-putcommit-not-write-once-01) · PR #208 prerequisite; #206 remains blocked until merge and rebase |
...
| `ISSUE-SYNC-RECVFS-NOT-WRITE-ONCE-01` | HIGH (P1) ✅ Fixed by PR #208 (pending merge) | `RecvFS` validates lowercase `fs_id`, compares logical SHA-1 identity across legacy/canonical layouts, completes pre-existing placeholders, rejects semantic conflicts, fails closed on storage errors, and avoids new child upserts/Paxos | [known issue](./KNOWN_ISSUES.md#issue-sync-recvfs-not-write-once-01) · PR #208 prerequisite; #206 remains blocked until merge and rebase |
