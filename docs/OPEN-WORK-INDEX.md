...
**Last updated:** 2026-09-19 (publish-repair dead-row retention contract)
...
| `ISSUE-PUBLISH-REPAIR-DEAD-ROW-RETENTION-01` | MEDIUM (P2) — OPEN — FOLLOW-UP / PRE-GC over-retention | A genuinely dead/unreachable publication attempt whose repair row survived request-local cleanup (proven loser + failed clear; pre-HEAD abort/rollback + failed clear; equivalent dead residues) is UNKNOWN on every visit and re-pins its staged blocks for 35 d indefinitely. Post-success clear failure is not this leak (REACHABLE, worker retries). No under-retention; does not block X1 safety closure or destructive-GC safety activation. Observed while adding repair-worker observability | [known issue](./KNOWN_ISSUES.md#issue-publish-repair-dead-row-retention-01) |
