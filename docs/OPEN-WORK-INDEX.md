...
**PC-D1B.4 status (2026-09-24):** the decision is now **CLOSED / MERGEABLE**: CW-M29 freezes the fleet-wide pairwise clock-skew lease and two-clock failure model, and CW-M30 freezes the exact UUIDv5 namespace/encoding/vector. CW-M27 and same-clock CW-M28 remain closed; canonical status and the P2 PR-sequencing contradiction are reconciled in [PC-D1B-CERTIFICATION-WINDOW-FENCE.md](./PC-D1B-CERTIFICATION-WINDOW-FENCE.md). PC-D1B.5 is an OPEN runtime follow-up, not a prerequisite to merge this decision PR; it is required before GC activation or a productive consumer. No runtime, schema, consumer, mapping-authority or GC change landed here.
...
| `ISSUE-PCD1B4-CERTIFICATION-WINDOW-FENCE-01` | HIGH (P1) — decision CLOSED / mergeable 2026-09-24; runtime OPEN (PC-D1B.5 follow-up, PRE-GC / pre-consumer) | Per-library E/P/S fence with generation ownership and gateway-only full-projection reaffirmation remains the architecture. CW-M27: every non-null S requires EACH_QUORUM even after UNKNOWN/local WRITETIME>S. CW-M28: no future tombstone relative to one clock. CW-M29 now freezes the fleet-wide `ClockSafetyLease` bound, clock-health/monotonicity requirements, and `safe_now = gc_local_now - Δ - 1us`; the runtime must fail closed and retain P when no safe interval or valid lease exists. CW-M30 freezes the exact UUIDv5 namespace, encoding and known vector. Also frozen: canonical `created_at` intent sentinel, whole-row W over all live cells, persisted durable `identity_at`, no v1 batch amortization, and repeat reaffirmation on UNKNOWN | [known issue](./KNOWN_ISSUES.md#issue-pcd1b4-certification-window-fence-01) · [PC-D1B.4](./PC-D1B-CERTIFICATION-WINDOW-FENCE.md) |
