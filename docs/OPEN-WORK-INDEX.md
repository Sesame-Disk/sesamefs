...
**Last updated:** 2026-09-23 (PC-D1B.4 certification-window fence decision)
**PC-D1B.4 status (2026-09-23):** the certification-window lifecycle fence is selected but remains **NO MERGE** pending closure of two P1 timestamp-proof findings. This round adds the mandatory global reaffirmation retry contract (CW-M27), forbids future destructive tombstones (CW-M28), and resolves the P2 intent-existence, whole-row W, durable `IdentityAt` and performance-wording contradictions in [PC-D1B-CERTIFICATION-WINDOW-FENCE.md](./PC-D1B-CERTIFICATION-WINDOW-FENCE.md). Runtime implementation remains PC-D1B.5; no runtime, schema, consumer, mapping-authority or GC change landed here.
...
| `ISSUE-PCD1B4-CERTIFICATION-WINDOW-FENCE-01` | HIGH (P1) — decision selected; 2 timestamp-proof blockers remain open; runtime OPEN (PC-D1B.5); PRE-GC and pre-consumer | Per-library E/P/S fence with generation-owned intents and gateway-only full-projection reaffirmation remains the architecture. Before merge/runtime use: any non-null S requires EACH_QUORUM reaffirmation of every covered row on every certifier attempt even after UNKNOWN/local WRITETIME>S (CW-M27); no destructive generation/tombstone may be ahead of wall clock, so future-W/epoch targets are postponed (CW-M28). Also frozen: observed E/P/S plus canonical `created_at` sentinel, whole-row W over all live cells, token from persisted durable `identity_at`, no v1 batch amortization, UNKNOWN may repeat reaffirmation | [known issue](./KNOWN_ISSUES.md#issue-pcd1b4-certification-window-fence-01) · [PC-D1B.4](./PC-D1B-CERTIFICATION-WINDOW-FENCE.md) |
