...
| `ISSUE-PCD1B4-CERTIFICATION-WINDOW-FENCE-01` | HIGH (P1) — decision closed 2026-09-23, runtime OPEN (PC-D1B.5); PRE-GC and pre-consumer | A witness can settle over, or survive, the destruction of the commit/fs_object/`fs:` state it certifies; the witness CAS sees only HEAD and `deleted_at`. Decided: per-library destruction epoch + generation-owned pending intents (token -> generation) in the global SERIAL HEAD domain for GC destroyers; proven-never-HEAD D4/D5 cleanups take a capability instead; witness stays `(H, V)` and is read at global SERIAL for productive use; lifecycle writes unchanged | [known issue](./KNOWN_ISSUES.md#issue-pcd1b4-certification-window-fence-01) · [PC-D1B.4](./PC-D1B-CERTIFICATION-WINDOW-FENCE.md) |
