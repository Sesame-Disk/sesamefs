...
| `ISSUE-PC0-INHERITED-DEPENDENCY-CONTINUITY-01` | HIGH (P1) | `PublishableInput`/the candidate coordinator boundary only require continuity proof for dependencies a HEAD will *newly* live on, not for dependencies inherited unchanged from the old HEAD; R3's own `LogicalPositiveBlockDelta` note already flags that gap and PC-0 explicitly records/discusses it but does not resolve it | [known issue](./KNOWN_ISSUES.md#issue-pc0-inherited-dependency-continuity-01) · [PC-0](./PUBLICATION-PROTOCOL-CHARACTERIZATION.md) |
...
| `ISSUE-LIBRARY-ROLLBACK-GHOST-PROJECTIONS-01` | MEDIUM — documented debt from H1 round 4 | `rollbackNewLibrary` takes authority with a conditional `DELETE` of the canonical row before its cleanup `LoggedBatch`; a crash or batch failure in between leaves `libraries_by_id` plus the active owner/org/global projections (which drive `CountActiveLibraries`, `ownerHasActiveLibraryNamed` and admin listings) behind a missing canonical row, and no existing reconciler covers active projections. Accepted in exchange for never destroying a HEAD another initializer published; needs a durable recovery seam + fault-injection test | [known issue](./KNOWN_ISSUES.md#issue-library-rollback-ghost-projections-01) |
