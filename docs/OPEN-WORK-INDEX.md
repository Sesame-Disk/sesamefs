...
| `ISSUE-LIBRARY-ROLLBACK-GHOST-PROJECTIONS-01` | MEDIUM ✅ **Resolved 2026-09-11** | Durable `library_rollback_pending` marker (no TTL, 32 recovery buckets) is written before the HEAD LWT; a Server-owned reaper re-enters `deleteUnpublishedLibraryRow` then idempotent derived cleanup. Marker is discovery only — never cleanup authority. Fault-injection: authority applied → crash → `RecoverPendingLibraryRollbacks` clears ghosts | [known issue](./KNOWN_ISSUES.md#issue-library-rollback-ghost-projections-01) |
