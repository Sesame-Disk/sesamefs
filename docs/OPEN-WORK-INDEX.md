...
| `ISSUE-LIBRARY-HEAD-SERIAL-DOMAIN-01` | MEDIUM (multi-DC only) ✅ **Resolved 2026-09-14** | The four canonical HEAD-authority LWTs pin `SerialConsistency(db.LibraryHeadSerialConsistency)` = global `SERIAL` and no longer inherit `serial_consistency`. Session default `LOCAL_SERIAL` is still allowed for other LWTs. PC-0 chain-pins the domain; mutation M1–M18 RED; real 3-DC concurrent advance + initial HEAD under session `LOCAL_SERIAL` have exactly one winner. Satisfies the PC-D1 global SERIAL prerequisite; certified baseline implementation remains OPEN | [known issue](./KNOWN_ISSUES.md#issue-library-head-serial-domain-01) |
