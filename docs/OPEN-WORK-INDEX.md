...
| `ISSUE-PUBLISH-REPAIR-RENEWAL-AFTER-CLASSIFY-01` | HIGH (P1) ✅ **Resolved 2026-09-14** — PRE-X1 / PRE-GC | Closes the classifier-induced gap: once the pre-classify renewal of `pub:<repo:commit:fsID>` (existing `StillPending → Add → StillPending` helper, compensation on gone or on partial fan-out failure) completes, the bounded walk cannot expire the pin; renewal failure fails closed without walking; one renewal per visit; a row cleared underneath the walk or underneath a failed positive settlement has its pin removed by the same visit, with an in-process bounded retry if that removal fails (residual vs `main`: a removal lost with the process, TTL-bounded); REACHABLE keeps `renew → classify → promote → remove pub: → delete`. Unit + M1–M12 mutations (43/43 RED) + real-Cassandra `renewal_before_classify` leg (external clear during walk, UNKNOWN, REACHABLE). Not closed: discovery-after-expiry and expiry during the sequential per-block fan-out (`ISSUE-PUBLISH-REPAIR-DISCOVERY-SCALE-01`, `ISSUE-GC-PUB-REF-ZERO-REF-01`), owned-pub cleanup race residual / no durable cleanup witness, R31/W2/GC | [known issue](./KNOWN_ISSUES.md#issue-publish-repair-renewal-after-classify-01) |
