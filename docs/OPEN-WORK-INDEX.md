...
| `ISSUE-SYNC-PUTCOMMIT-NOT-WRITE-ONCE-01` | HIGH (P1) | `PutCommit` plain-upserted an existing `commit_id`, so the root/parent validated by a HEAD publisher could change below the same identity | [known issue](./KNOWN_ISSUES.md#issue-sync-putcommit-not-write-once-01) · prerequisite `Sync content-addressed identity hardening` - blocks #206 current end-to-end claim |
...
| `ISSUE-SYNC-RECVFS-NOT-WRITE-ONCE-01` | HIGH (P1) | `RecvFS` trusted wire `fs_id` and plain-inserted `fs_objects`, so the same identity could mutate `block_ids` below a published tree | [known issue](./KNOWN_ISSUES.md#issue-sync-recvfs-not-write-once-01) · prerequisite `Sync content-addressed identity hardening` |
