...
| `ISSUE-PCD1B-METADATA-IDENTITY-AUTHORITY-01` | HIGH (P1) — OPEN, decision recorded 2026-09-20 | Baseline certification resolves `commits` / `fs_objects` and SHA-1-only block dependencies (`block_id_mappings`) through ordinary reads, so a complete but unproven or divergent historical identity can be certified. Deletes on four production paths are unfenced, and a covered identity removed between the final revalidation and witness settlement would settle a witness that is already false. Claims must pin the canonical global `SERIAL` domain, never the configurable session default. Minimum certifier correctness (refuse the unproven) is separate from legacy reach, which is its own work item | [known issue](./KNOWN_ISSUES.md#issue-pcd1b-metadata-identity-authority-01) · [PC-D1B](./PC-D1B-METADATA-IDENTITY-AUTHORITY.md) |
