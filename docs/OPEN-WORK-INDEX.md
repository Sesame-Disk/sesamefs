...
| `ISSUE-PCD1-CERTIFIED-BASELINE-IMPLEMENTATION-01` | HIGH (P1) | Durable certified-through-HEAD witness, historical certification/backfill, and atomic HEAD+witness CAS are not implemented; implementation must use exact physical incarnation `P` -> non-expiring current-library liveness -> exact-P/GC-authority revalidation, reject TTL-only witnesses and legacy deterministic locators until safely rematerialized to minted P, and use one compatible global `SERIAL` Paxos domain (current HEAD writers already pin that domain; `ISSUE-LIBRARY-HEAD-SERIAL-DOMAIN-01` closed); required before PC-2 | [known issue](./KNOWN_ISSUES.md#issue-pcd1-certified-baseline-implementation-01) · [PC-D1](./PC-D1-INHERITED-DEPENDENCY-CONTINUITY.md) |
...
| `ISSUE-LIBRARY-HEAD-SERIAL-DOMAIN-01` | MEDIUM (multi-DC only) ✅ **Resolved 2026-09-14** | The four canonical HEAD-authority LWTs pin `SerialConsistency(db.LibraryHeadSerialConsistency)` = global `SERIAL` and no longer inherit `serial_consistency`. Session default `LOCAL_SERIAL` is still allowed for other LWTs. PC-0 chain-pins the domain; mutation M1–M28 RED; real 3-DC concurrent advance + initial HEAD under session `LOCAL_SERIAL` have exactly one winner. Satisfies the PC-D1 global SERIAL prerequisite; certified baseline implementation remains OPEN | [known issue](./KNOWN_ISSUES.md#issue-library-head-serial-domain-01) |
...
| `ISSUE-PUBLISH-REPAIR-RENEWAL-AFTER-CLASSIFY-01` | HIGH (P1) ✅ **Resolved 2026-09-14** — PRE-X1 / PRE-GC | Closes the classifier-induced gap: once the pre-classify renewal of `pub:<repo:commit:fsID>` (existing renewal helper: `StillPending → intent → per-block fan-out → ARM → gone-check`, compensation on gone or on partial fan-out failure) completes, the bounded walk cannot expire the pin; renewal failure fails closed without walking; one renewal per visit; a write-ahead, producer-token-keyed, TTL-free cleanup intent (`published_repair_liveness_cleanups`, migration 025) is written PREPARING with a lease before every pin, every pin and every cleanup of that producer carry the lease as their Cassandra timestamp (a late write is shadowed by its tombstone), the lease is renewed inside the fan-out by a conditional LWT, the producer ARMs it afterwards by a conditional payload-carrying LWT, and the sweep consumes it only once finished — armed by the producer, or claimed by the sweep through an exact-lease **freeze CAS** that is mutually exclusive with the producer's EXTEND/ARM (an expired lease read from a listing is never authority by itself; every transition of the row, INSERT included, is a Paxos CAS) — never without payload or lease, compacting finished producers (frozen ones included, so abandoned PREPARING intents stay bounded) per pending identity; a producer that loses EXTEND/ARM is decided by the row (gone → compensate; pending → retain without removing pins); a retired witness is CONSUMED, re-fenced with an `EACH_QUORUM` tombstone every 3d for the 35d pin TTL (`refenced_at` advances only after every DC acknowledged) and deleted (SERIAL `IF EXISTS`) only after a mandatory final fence, so the fence outlives `gc_grace_seconds` (pinned by migration 026); the destructive absence check (visit and sweep) retains on the session read and escalates a local absence to an `EACH_QUORUM` authority read (fail closed), so a row cleared underneath the walk or underneath a failed positive settlement has its pin removed by the same visit or, after any failure or process loss, by the sweep — no ownerless pin added versus `main`, no local absence can remove liveness, no witness consumed while its producer can still write, no stale-lease consumption; REACHABLE keeps `renew → classify → promote → fence pub: → retire intent to CONSUMED → delete row`; a CONSUMED witness fences only while its identity is conclusively gone (a pending requeue keeps it untouched) and compaction is a CAS on the listed snapshot. Unit (incl. M28–M30 freeze/fence/compaction and M33/M34 re-fence models) + M1–M38 mutations (73/73 RED) + real-Cassandra `renewal_before_classify` leg (external clear during walk, UNKNOWN, REACHABLE, durable rediscovery by the sweep) + real 3-DC legs (blind-DC sweep keeps the pin; unavailable DC fails closed; a `dc-na` sweeper with a stale PREPARING(L1) snapshot loses the freeze to a `dc-eu` EXTEND L1→L2 and removes nothing, then wins past L2 and fences the producer; a CONSUMED witness refuses to record a re-fence or its final fence while a DC is down and advances only through a globally acknowledged tombstone once every DC is back, and a pending requeue of the identity in another DC blocks it). Not closed: discovery-after-expiry and expiry during the sequential per-block fan-out (`ISSUE-PUBLISH-REPAIR-DISCOVERY-SCALE-01`, `ISSUE-GC-PUB-REF-ZERO-REF-01`), the pre-existing gone-read→DELETE requeue race, R31/W2/GC | [known issue](./KNOWN_ISSUES.md#issue-publish-repair-renewal-after-classify-01) |
...
| `ISSUE-BLOCK-REFERENCES-GC-GRACE-CERTIFICATION-01` | MEDIUM (P2) â PRE-GC follow-up, not a #220 blocker | The repair cleanup re-fence interval (3d) is certified against `gc_grace_seconds = 864000` pinned by migration 026 and a Go guard, not against the live `system_schema.tables`; an operator lowering it later silently shortens the fence (over-retention only). Intended fix: readiness gate reading the table property | [known issue](./KNOWN_ISSUES.md#issue-block-references-gc-grace-certification-01) |
