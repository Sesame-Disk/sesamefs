...
| `ISSUE-PCD1B-MAPPING-PROJECTION-STABILITY-01` | HIGH (P1) — PRE-CONSUMER | After #233 a witness can rest on mapping authority A while the mutable `block_id_mappings` row later resolves to B for ordinary readers; no destruction happens, so the PC-D1B.4 fence does not see it | [known issue](./KNOWN_ISSUES.md#issue-pcd1b-mapping-projection-stability-01) |
