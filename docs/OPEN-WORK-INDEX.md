...
**PC-D1B.4 status (2026-09-24):** **DECISION CLOSED / MERGEABLE.** CW-M33's accepted-Paxos pause/barrier race and G21 barrier-removal mutation are green/red as required on the isolated Cassandra 5.0.9 test image. CW-M31 remains closed for cross-DC visibility, with stable absence now closed by the SERIAL-settlement plus EACH_QUORUM proof contract. CW-M34 freezes the in-flight writer lifetime property and has model plus real 3-DC success-but-hidden characterization; runtime permits/recovery remain PC-D1B.5. CW-M27..CW-M34 decision findings are closed; PC-D1B.5 runtime remains required before GC activation or a productive consumer. No runtime, schema, consumer, mapping-authority or GC change landed here.
...
| `ISSUE-PCD1B4-CERTIFICATION-WINDOW-FENCE-01` | HIGH (P1) — decision CLOSED / mergeable 2026-09-24; runtime OPEN (PC-D1B.5, PRE-GC / pre-consumer) | Per-library E/P/S fence remains the architecture. CW-M27..CW-M34 decision contracts are closed. CW-M31 proves cross-DC visibility; CW-M33 additionally requires a global-SERIAL accepted-Paxos settlement barrier before EACH_QUORUM absence, proven by the isolated Cassandra 5.0.9 pause race and G21 mutation RED. CW-M34 requires no admitted write at `w <= g` remain able to succeed hidden after GC; model, G20 mutation and 3-DC success-but-hidden characterization exist, implementation remains PC-D1B.5. Also frozen: `ClockSafetyLease`, UUIDv5 vectors, canonical `created_at` sentinel, whole-row W, persisted durable `identity_at`, no v1 batch amortization, and repeat reaffirmation on UNKNOWN | [known issue](./KNOWN_ISSUES.md#issue-pcd1b4-certification-window-fence-01) · [PC-D1B.4](./PC-D1B-CERTIFICATION-WINDOW-FENCE.md) |
