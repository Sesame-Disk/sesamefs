...
**Last updated:** 2026-09-24 (PC-D1B.4 final in-flight authority evidence)
**PC-D1B.4 status (2026-09-24):** **DECISION CLOSED / MERGEABLE.** CW-M33's accepted-Paxos pause/barrier race, post-barrier SERIAL-present issuance race, and G21/G22 mutations are green/red as required on the isolated Cassandra 5.0.9 test image. CW-M31 remains closed for cross-DC visibility, with stable absence now requiring both SERIAL to observe absence and the EACH_QUORUM recheck. CW-M34 freezes the in-flight writer lifetime property and has model plus real 3-DC success-but-hidden characterization; runtime permits/recovery remain PC-D1B.5. CW-M27..CW-M34 decision findings are closed; PC-D1B.5 runtime remains required before GC activation or a productive consumer. §14/§16 now include the SERIAL+EACH_QUORUM proof cost and distinguish normal local writer lifetime accounting from exceptional drain/recovery I/O. No runtime, schema, consumer, mapping-authority or GC change landed here.
**PR #231 closure status (2026-09-22):** the final Docker full integration profile and the short Go suite pass. Real-Cassandra evidence covers nullable fs_objects replay, placeholder completion, directory retry and delete, SHA1-only delete, zero-block delete, and durable claim retention. The mutation runner confirms the projection-access and dynamic-CQL bypass cases B22-B24 turn RED. The local profile skips tests requiring separate 3-DC host variables; see the PR #231 closure record in CURRENT_WORK.md. No #228, mapping-authority, or GC work was added to this PR.
...
- `ISSUE-PUBLISH-REPAIR-GONE-CHECK-XDC-AUTHORITY-01` — **Fixed 2026-09-19**;
  the post-renewal gone-check no longer removes the repair-owned `pub:` from
  a non-authoritative local absence. The existing 35-day TTL bounds any
  over-retention; renewal-after-classify, the concurrent settlement race, X1
  and GC remain separate.
...
X1 closure. W1 BorrowedFS own-liveness through HEAD is implemented and evidence-backed; W2/R31 still covers full writer continuity. PC-0 ([PUBLICATION-PROTOCOL-CHARACTERIZATION.md](./PUBLICATION-PROTOCOL-CHARACTERIZATION.md)) characterizes the current publication protocol and recommends a future multi-DC PublicationCoordinator; it does not close W2/R31. PC-1 (2026-09-11) introduced the coordinator skeleton and the common publication types in `internal/publication` (attempt identity, tri-state HEAD outcome, settlement disposition, opaque evidence boundary) with zero funnels migrated, zero productive importers, and zero runtime/schema/CL/TTL change; it does not execute any publication sequence and does not close W2/R31. PC-D1 (2026-09-12) closes the inherited-dependency architecture decision (`ISSUE-PC0-INHERITED-DEPENDENCY-CONTINUITY-01`): the certified baseline frontier owns continuity, and `WorkSetScopeNewlyLive` is incremental only with a valid durable witness. PC-D1A authority foundation (canonical witness columns, fail-closed validity, HEAD-fenced baseline CAS, and atomic global-SERIAL HEAD+witness authority) landed 2026-09-19. PR #228 implements the exact-P/GC-aware fail-closed certifier; PR #233 adds byte-proven PC-D1B.3 mapping authority and cold-path M18/M19 promotion. Lifecycle fencing, the first productive consumer, and PC-2 remain future work. No funnel is migrated. PC-D1B (2026-09-20) adds a distinct metadata prerequisite ahead of the certifier (`ISSUE-PCD1B-METADATA-IDENTITY-AUTHORITY-01`, decided in [PC-D1B-METADATA-IDENTITY-AUTHORITY.md](./PC-D1B-METADATA-IDENTITY-AUTHORITY.md)). The order is authority-only identity primitive (#230), writer/deleter wiring (#231), then PR #228 read-only certifier consumption. The certifier verifies the captured commit and every reachable fs_object against durable claims, preserves canonical fs_id keys byte-for-byte, accepts the authority-verified zero-block Cassandra representation, and revalidates source identity before witness settlement; it returns `identity_unproven` without a witness for SHA-1-only identities needing unauthoritative mapping. PC-D1B.3 mapping authority and cold-path M18/M19 promotion are implemented in PR #233. The certification-window fence is decided by PC-D1B.4 (per-library destruction epoch plus pending destruction intents predicated by the witness CAS; `ISSUE-PCD1B4-CERTIFICATION-WINDOW-FENCE-01`) and its runtime (PC-D1B.5) is required before destructive GC and the first productive consumer. Historical backfill remains a greenfield non-goal; no productive consumer or PC-2 work has started; preserve GC_ENABLED=false.
...
unchanged; physical S3 deletion remains out of scope). G4/G5, the P4c-orphan
logical-block PK replacement, and X1 remain open.
...
| `ISSUE-SYNC-PUTCOMMIT-NOT-WRITE-ONCE-01` | HIGH (P1) ✅ Fixed by merged PR #208 | `PutCommit` now arbitrates immutable (parent_id, root_fs_id) with first-writer-wins LWT; identical retries are idempotent and conflicts are rejected | [known issue](./KNOWN_ISSUES.md#issue-sync-putcommit-not-write-once-01) · PR #208 merged; #206 is rebased and being reaudited for its scoped W2 follow-ups |
| `ISSUE-PUBLISH-REPAIR-DISCOVERY-SCALE-01` | MEDIUM (P2) — also PRE-GC liveness | UNKNOWN repair rows are rediscovered by a full repair-bucket scan every minute; there is no hard time-to-visit bound, so renewal cannot prove TTL continuity. Durable discovery needs a separately audited bounded design | [known issue](./KNOWN_ISSUES.md#issue-publish-repair-discovery-scale-01) - provisional future PR, still unstarted |
...
| `ISSUE-PC0-EXACT-P-FUNNEL-GAP-01` | HIGH (P1) | Publication-authority/continuity before HEAD is not uniform by provenance: only CreateFileFromBlocks placements and Sync-provenanced blocks re-validate exact P; `UploadFile` passes `nil` into the shared finalizer. Not a prescription that every funnel must run exact-P | [known issue](./KNOWN_ISSUES.md#issue-pc0-exact-p-funnel-gap-01) · [PC-0](./PUBLICATION-PROTOCOL-CHARACTERIZATION.md) |
| `ISSUE-PC0-INHERITED-DEPENDENCY-CONTINUITY-01` | DECIDED / implementation follow-up | PC-D1 assigns inherited continuity to the certified baseline frontier; `WorkSetScopeNewlyLive` is incremental only with a valid witness, and absent/stale evidence fails closed to baseline certification | [known issue](./KNOWN_ISSUES.md#issue-pc0-inherited-dependency-continuity-01) · [PC-D1](./PC-D1-INHERITED-DEPENDENCY-CONTINUITY.md) |
| `ISSUE-PCD1-CERTIFIED-BASELINE-IMPLEMENTATION-01` | HIGH (P1) — OPEN, PC-D1A authority foundation and #228 certifier gate landed | PR #231 wires supported commits/fs_objects writers and deleters through the identity gateway. PR #228 verifies the captured commit and every reachable fs_object against durable claims before physical/liveness work, revalidates source identity before witness settlement, and fails closed for mapping-dependent SHA-1-only files. Remaining: the certification-window fence runtime (decided by PC-D1B.4, implemented by PC-D1B.5), first productive consumer, PC-2, and pre-GC Phase 5 work. Historical backfill is a greenfield non-goal; preserve `GC_ENABLED=false` | [known issue](./KNOWN_ISSUES.md#issue-pcd1-certified-baseline-implementation-01) · [PC-D1](./PC-D1-INHERITED-DEPENDENCY-CONTINUITY.md)
| `ISSUE-PCD1B4-CERTIFICATION-WINDOW-FENCE-01` | HIGH (P1) — decision CLOSED / mergeable 2026-09-24; runtime OPEN (PC-D1B.5, PRE-GC / pre-consumer) | Per-library E/P/S fence remains the architecture. CW-M27..CW-M34 decision contracts are closed. CW-M31 proves cross-DC visibility. CW-M33 requires the SERIAL proof-read itself to observe absence, plus the EACH_QUORUM recheck; the isolated Cassandra 5.0.9 tests cover pre-existing and post-barrier accepted Paxos, with G21/G22 removal mutations RED. CW-M34 requires no admitted write at `w <= g` remain able to succeed hidden after GC; model, G20 mutation and 3-DC success-but-hidden characterization exist, implementation remains PC-D1B.5. §14/§16 account for SERIAL+EACH_QUORUM proof reads and lifetime tracking/recovery cost. Also frozen: `ClockSafetyLease`, UUIDv5 vectors, canonical `created_at` sentinel, whole-row W, persisted durable `identity_at`, no v1 batch amortization, and repeat reaffirmation on UNKNOWN | [known issue](./KNOWN_ISSUES.md#issue-pcd1b4-certification-window-fence-01) · [PC-D1B.4](./PC-D1B-CERTIFICATION-WINDOW-FENCE.md) |
| `ISSUE-PCD1B-MAPPING-PROJECTION-STABILITY-01` | HIGH (P1) ✅ Closed in PR #233 (2026-09-25) | The dominant timestamp freeze is verified with `EACH_QUORUM`, productive resolution pins `LOCAL_QUORUM`, the production mutation inventory prohibits higher-timestamp ordinary writes and deletes, and M20/T2/H3/DEL1 turn RED if these guarantees regress | [known issue](./KNOWN_ISSUES.md#issue-pcd1b-mapping-projection-stability-01) · [PR #233](https://github.com/Sesame-Disk/sesamefs/pull/233)
| `ISSUE-PCD1B-CONTINUITY-LWT-GHOST-ROW-01` | LOW — PRE-GC | Any continuity LWT (witness CAS today, destruction intent in PC-D1B.5) whose ballot is later than a concurrent plain hard delete leaves a HEAD-less `libraries` row: never a valid witness, but GC's canonical-existence checks see a library | [known issue](./KNOWN_ISSUES.md#issue-pcd1b-continuity-lwt-ghost-row-01) |
| `ISSUE-PCD1B-STALE-TOMBSTONE-DISPLAY-METADATA-01` | MEDIUM — PRE-GC | A superseded destruction generation's row tombstone can shadow fs_object display metadata that reaffirmation (identity projection only) does not restore | [known issue](./KNOWN_ISSUES.md#issue-pcd1b-stale-tombstone-display-metadata-01) |
| `ISSUE-GC-HARD-DELETE-LEASE-SERIAL-DOMAIN-01` | MEDIUM — PRE-GC multi-DC | The library hard-delete lease LWTs inherit `serial_consistency`; under `LOCAL_SERIAL` a restore and a cascade in different DCs can both own the lease | [known issue](./KNOWN_ISSUES.md#issue-gc-hard-delete-lease-serial-domain-01) |
| `ISSUE-GC-HARD-DELETE-LEASE-NONFENCING-01` | HIGH (P1) — CURRENT-RUNTIME / FOLLOW-UP; also required PRE-GC; pre-existing, not a #232 blocker | The authenticated `PermanentDeleteRepo` API synchronously hard-deletes library rows without a `GC_ENABLED` gate; a stale lease owner can resume after restore/delete takeover and apply its unconditional batch | [known issue](./KNOWN_ISSUES.md#issue-gc-hard-delete-lease-nonfencing-01) |
| `ISSUE-PCD1B-AUTHORITY-CLAIM-RETIREMENT-01` | MEDIUM (P2) — accepted debt, registered 2026-09-20 | Identity authority claims are write-once and non-expiring and survive deletion of their source row, so claims for failed initializations, losing commits and deleted identities/libraries accumulate with no retirement protocol. Greenfield cost, not historical residue; not a merge precondition for #228/#229 | [known issue](./KNOWN_ISSUES.md#issue-pcd1b-authority-claim-retirement-01) · [PC-D1B](./PC-D1B-METADATA-IDENTITY-AUTHORITY.md) |
...
| `ISSUE-LIBRARY-HEAD-SERIAL-DOMAIN-01` | MEDIUM (multi-DC only) ✅ **Resolved 2026-09-14** | The six canonical HEAD-authority LWTs (four legacy writers plus the two PC-D1A authority-only primitives) pin `SerialConsistency(db.LibraryHeadSerialConsistency)` = global `SERIAL` and no longer inherit `serial_consistency`. Session default `LOCAL_SERIAL` is still allowed for other LWTs. PC-0 chain-pins the domain; mutation M1–M28 RED plus PC-D1A's 13 directed mutation legs RED; real 3-DC concurrent advance + initial HEAD under session `LOCAL_SERIAL` have exactly one winner. Satisfies the PC-D1 global SERIAL prerequisite; PC-D1A foundation landed and PR #228 implemented the fail-closed certifier gate; mapping promotion is implemented by PR #233 (pending merge), while productive frontier activation remains OPEN | [known issue](./KNOWN_ISSUES.md#issue-library-head-serial-domain-01) |
| `ISSUE-LIBRARY-INITIAL-HEAD-CONCURRENCY-01` | HIGH (P1) ✅ **Resolved 2026-09-11** | HEAD initialization now goes through `FSHelper.InitializeLibraryHeadIfUnset` (`IF head_commit_id = null AND created_at != null`); the two unconditional initializers are gone, `GET /commit/HEAD` returns the Paxos-settled HEAD from a blind DC instead of overwriting it; unit + single-cluster + real 3-DC handler-level evidence; `TestPC0NoUnconditionalHeadUpdateRemains` and mutation leg M10 pin the resolution; the blind-DC guarantee is stated for the shipped global `SERIAL` domain (`ISSUE-LIBRARY-HEAD-SERIAL-DOMAIN-01` closed 2026-09-14: HEAD LWTs pin global SERIAL even when the session default is `LOCAL_SERIAL`). Fourth round: `rollbackNewLibrary` takes authority with `DELETE ... IF head_commit_id = null` so a creator's own failure cannot destroy a HEAD another initializer published. `PublicationCoordinator` prerequisite met. Second review round (same day) closed 4 more runtime gaps: v4 CAS-write-timeout misclassification, creation-handler `503 Retry-After` that promised a resumption it could not deliver (now an honest `500` + preserved `repo_id`; durable resumption split out as `ISSUE-GROUP-LIBRARY-CREATION-RESUMABILITY-01`), KNOWN_LOSER cleanup ordering, and unattributed commit rows on definitive rejection | [known issue](./KNOWN_ISSUES.md#issue-library-initial-head-concurrency-01) · [PC-0 §3.4](./PUBLICATION-PROTOCOL-CHARACTERIZATION.md) · `scripts/h1-initial-head-multidc-validation.sh` |
...
| `ISSUE-PC0-CONTENT-RESURRECTION-PUBLICATION-01` | HIGH (P1) | `RevertFile`/`RevertDirectory`/`RestoreTrashItem`/`RevertDirents` publish a positive borrowed block-dependency delta with no pin, `pub:`, repair, or fence; reclassified from "tree-only" by PC-0; fix belongs to W2 / funnel migration | [known issue](./KNOWN_ISSUES.md#issue-pc0-content-resurrection-publication-01) · [PC-0 §3.5](./PUBLICATION-PROTOCOL-CHARACTERIZATION.md) |
...
| `ISSUE-PUBLISH-REPAIR-RENEWAL-AFTER-CLASSIFY-01` | HIGH (P1) — **OPEN** — PRE-X1 / PRE-GC, not an R31-C1 blocker | An unresolved visit can write/refresh per-row `pub:` after classify while pending; that can recreate an expired pin. The hazard is the zero-ref interval, not inability to renew. Discovery-after-expiry remains `ISSUE-PUBLISH-REPAIR-DISCOVERY-SCALE-01`. **PR #220 and PR #222 rejected (closed without merge, nothing in `main`)**: a design proof (main-liveness non-regression, bounded durable state under unlimited retries, adversarial matrix) is required before the next runtime attempt — [PUBLISH-REPAIR-LIVENESS-REJECTED-DESIGNS.md](./PUBLISH-REPAIR-LIVENESS-REJECTED-DESIGNS.md). **PR #224 design proof: candidate V0 (stable-pin refresh as a pre-classifier fan-out) rejected, outcome B**; residual **parked** (blocks X1 closure and GC activation on this guarantee, not development); next: repair-worker observability → fail-closed GC health gate decision — [R31-REPAIR-LIVENESS-DESIGN-PROOF.md](./R31-REPAIR-LIVENESS-DESIGN-PROOF.md) | [known issue](./KNOWN_ISSUES.md#issue-publish-repair-renewal-after-classify-01) |
| `ISSUE-PUBLISH-REPAIR-DEAD-ROW-RETENTION-01` | MEDIUM (P2) — OPEN — FOLLOW-UP / GENERAL over-retention | A genuinely dead/unreachable publication attempt whose repair row survived request-local cleanup (proven loser + failed clear; pre-HEAD abort/rollback + failed clear; equivalent dead residues) has no terminal settlement path: it stays discoverable and is revisited under retry/backoff; successful unresolved visits can repeatedly refresh the 35 d `pub:`, so over-retention can continue indefinitely. Post-success clear failure is not this leak (REACHABLE, worker retries). No under-retention; does not block X1 safety closure or destructive-GC activation. Observed while adding repair-worker observability | [known issue](./KNOWN_ISSUES.md#issue-publish-repair-dead-row-retention-01) |
| `ISSUE-PUBLISH-REPAIR-CROSS-CHUNK-CYCLE-01` | MEDIUM (P2) — follow-up, not an R31-C1 blocker | Resumed reachability chunks seed only the anchored HEAD, so a corrupt cycle longer than one chunk that lies entirely below HEAD rotates the cursor forever (UNKNOWN/retain, never cleanup). Cycles through HEAD and cycles within one chunk are detected | [known issue](./KNOWN_ISSUES.md#issue-publish-repair-cross-chunk-cycle-01) |
| `ISSUE-PUBLISH-REPAIR-PROGRESS-PAXOS-DOMAIN-01` | MEDIUM (P2) — follow-up, not an R31-C1 safety blocker | Progress SERIAL LWTs live on the 32-bucket discovery table and mix with ordinary INSERT/DELETE; unrelated repairs in one bucket share a Paxos domain. `created_at` is millisecond TIMESTAMP, not a UUID generation. Intended fix is a per-repair LWT-only progress table | [known issue](./KNOWN_ISSUES.md#issue-publish-repair-progress-paxos-domain-01) |
| `ISSUE-PUBLISH-HEAD-TREE-STATS-COST-01` | P2 (performance) | Every HEAD publish walks the whole directory tree for stats inside the stage→HEAD window; Sync does it twice before its CAS; PC-0 §12 corrected, optimization is a follow-up | [known issue](./KNOWN_ISSUES.md#issue-publish-head-tree-stats-cost-01) |
| `ISSUE-PUBLISH-REPAIR-TIMEOUT-CLEANUP-01` | ✅ CLOSED 2026-09-06 (W2 authority defect) | Timeout/lease is no longer cleanup authority in shared W2 repair; R31 residuals remain under `ISSUE-GC-PUB-REF-ZERO-REF-01` and `ISSUE-PUBLISH-REPAIR-KNOWN-LOSER-DURABILITY-01`; the shared repair reachability classifier is closed by #213 while broader R31 remains open | [known issue](./KNOWN_ISSUES.md#issue-publish-repair-timeout-cleanup-01) · [zero-ref](./KNOWN_ISSUES.md#issue-gc-pub-ref-zero-ref-01) · [reachability](./KNOWN_ISSUES.md#issue-publish-repair-reachability-01) · [known loser](./KNOWN_ISSUES.md#issue-publish-repair-known-loser-durability-01) |
| `ISSUE-PUBLISH-REPAIR-REACHABILITY-01` | ✅ CLOSED 2026-09-10 (shared repair classifier) | Tri-state classification now uses a 30s/1024-node cold-path bound, `SERIAL` HEAD and `EACH_QUORUM` commit reads; inconclusive evidence never authorizes cleanup. No durable negative producer was invented | [known issue](./KNOWN_ISSUES.md#issue-publish-repair-reachability-01) · broader [R31](./GC-X1-PHYSICAL-LIFE-HANDOFF-PLAN.md#16-r31-remains-a-blocker-open) remains open |
