...
| `ISSUE-LIBRARY-HEAD-SERIAL-DOMAIN-01` | MEDIUM (multi-DC only) ✅ **Resolved 2026-09-14** | The six canonical HEAD-authority LWTs (four legacy writers plus the two PC-D1A authority-only primitives) pin `SerialConsistency(db.LibraryHeadSerialConsistency)` = global `SERIAL` and no longer inherit `serial_consistency`. Session default `LOCAL_SERIAL` is still allowed for other LWTs. PC-0 chain-pins the domain; mutation M1–M28 RED plus PC-D1A's seven mutation legs RED; real 3-DC concurrent advance + initial HEAD under session `LOCAL_SERIAL` have exactly one winner. Satisfies the PC-D1 global SERIAL prerequisite; PC-D1A foundation landed, while certifier/backfill/frontier activation remain OPEN | [known issue](./KNOWN_ISSUES.md#issue-library-head-serial-domain-01) |
