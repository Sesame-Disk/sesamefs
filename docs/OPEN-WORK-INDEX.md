...
| `ISSUE-PUBLISH-REPAIR-RENEWAL-AFTER-CLASSIFY-01` | HIGH (P1) ✅ **Resolved 2026-09-14** — PRE-X1 / PRE-GC | A visit that finds a live durable repair now renews `pub:<repo:commit:fsID>` (existing `StillPending → Add → StillPending` helper, exact compensation) **before** the bounded classifier; renewal failure fails closed without walking; one renewal per visit; REACHABLE keeps `renew → classify → promote → remove pub: → delete`. Unit + M1–M8 mutations (39/39 RED) + real-Cassandra `renewal_before_classify` leg. Not closed: discovery-after-expiry (`ISSUE-PUBLISH-REPAIR-DISCOVERY-SCALE-01`, `ISSUE-GC-PUB-REF-ZERO-REF-01`), owned-pub cleanup race, R31/W2/GC | [known issue](./KNOWN_ISSUES.md#issue-publish-repair-renewal-after-classify-01) |
