...
| `ISSUE-PUBLISH-REPAIR-RENEWAL-AFTER-CLASSIFY-01` | HIGH (P1) ✅ **Resolved 2026-09-17** (PR #222; PR #220's durable-witness protocol abandoned, recorded as a rejected approach) — PRE-X1 / PRE-GC | Closes the classifier-induced zero-ref interval with `main`'s flow plus one write-only step: a transient walk pin `pub:<repo:commit:fsID>:walk` (1h TTL, distinct referrer, stable per row, never removed) written before the bounded classifier; the 35d renewal stays after the walk as in `main`, its compensation decides absence at `EACH_QUORUM`. Crash residual bounded to one 1h walk reference per block; no schema, no durable per-visit state. M1–M15 mutations (46/46 RED), real-Cassandra `renewal_before_classify` leg, 3-DC `main` legs + directed cleanup-authority leg. Discovery-after-expiry and expiry during the walk-pin fan-out remain separate R31 follow-ups | [known issue](./KNOWN_ISSUES.md#issue-publish-repair-renewal-after-classify-01) |
