...
| `ISSUE-PCD1B4-WITNESS-GHOST-ROW-01` | LOW — PRE-GC | A witness CAS whose ballot is later than a concurrent plain hard delete leaves a witness-only `libraries` row (HEAD null): never a valid witness, but GC's canonical-existence checks see a library | [known issue](./KNOWN_ISSUES.md#issue-pcd1b4-witness-ghost-row-01) |
