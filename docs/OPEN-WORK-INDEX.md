...
| `ISSUE-PUBLISH-REPAIR-RENEWAL-AFTER-CLASSIFY-01` | HIGH (P1) ✅ **Resolved 2026-09-17** (minimal fix; PR #220 witness protocol abandoned, recorded as a rejected approach) — PRE-X1 / PRE-GC | Closes the classifier-induced gap: once the pre-classify renewal of `pub:<repo:commit:fsID>` (existing `StillPending → Add → StillPending` helper, compensation on gone or on partial fan-out failure) completes, the bounded walk cannot expire the pin; renewal failure fails closed without walking; one renewal per visit; a row cleared underneath the walk has its pin removed by the same visit (no widening of the cleanup race vs `main`); REACHABLE keeps `renew → classify → promote → remove pub: → delete`. Unit + M1–M10 mutations (41/41 RED) + real-Cassandra `renewal_before_classify` leg (external clear during walk, UNKNOWN, REACHABLE). Not closed: discovery-after-expiry and expiry during the sequential per-block fan-out (`ISSUE-PUBLISH-REPAIR-DISCOVERY-SCALE-01`, `ISSUE-GC-PUB-REF-ZERO-REF-01`), owned-pub cleanup race residual, R31/W2/GC | [known issue](./KNOWN_ISSUES.md#issue-publish-repair-renewal-after-classify-01) |
