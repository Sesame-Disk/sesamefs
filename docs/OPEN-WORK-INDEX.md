...
**Last updated:** 2026-09-24 (PC-D1B.4 cross-node clock audit)
**PC-D1B.4 status (2026-09-24):** the decision is **CLOSED / MERGEABLE**: CW-M29 freezes the fleet-wide pairwise clock-skew lease/two-clock model; CW-M30 freezes block, commit and fs_object UUIDv5 vectors; CW-M31 requires global EACH_QUORUM absence proof; CW-M32 requires writer-side clock-health admission. CW-M27 and same-clock CW-M28 remain closed. Canonical status and PR sequencing are reconciled in [PC-D1B-CERTIFICATION-WINDOW-FENCE.md](./PC-D1B-CERTIFICATION-WINDOW-FENCE.md). PC-D1B.5 is an OPEN runtime follow-up, not a prerequisite to merge this decision PR; it is required before GC activation or a productive consumer. No runtime, schema, consumer, mapping-authority or GC change landed here.
...
| `ISSUE-PCD1B4-CERTIFICATION-WINDOW-FENCE-01` | HIGH (P1) — decision CLOSED / mergeable 2026-09-24; runtime OPEN (PC-D1B.5 follow-up, PRE-GC / pre-consumer) | Per-library E/P/S fence with generation ownership and gateway-only full-projection reaffirmation remains the architecture. CW-M27: every non-null S requires EACH_QUORUM even after UNKNOWN/local WRITETIME>S. CW-M28: no future tombstone relative to one clock. CW-M29 freezes the fleet-wide `ClockSafetyLease` bound and safe timestamp margin. CW-M30 freezes namespace/encoding vectors for block, commit and fs_object items. CW-M31 requires a global EACH_QUORUM absence proof; local absence is not authority. CW-M32 requires every supported writer timestamp source to obey clock-health admission. Also frozen: canonical `created_at` intent sentinel, whole-row W, persisted durable `identity_at`, no v1 batch amortization, and repeat reaffirmation on UNKNOWN | [known issue](./KNOWN_ISSUES.md#issue-pcd1b4-certification-window-fence-01) · [PC-D1B.4](./PC-D1B-CERTIFICATION-WINDOW-FENCE.md) |
