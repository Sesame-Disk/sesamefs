...
| `ISSUE-PUBLISH-REPAIR-RENEWAL-AFTER-CLASSIFY-01` | HIGH (P1) — **OPEN** — PRE-X1 / PRE-GC, not an R31-C1 blocker | An unresolved visit can write/refresh per-row `pub:` after classify while pending; that can recreate an expired pin. The hazard is the zero-ref interval, not inability to renew. Discovery-after-expiry remains `ISSUE-PUBLISH-REPAIR-DISCOVERY-SCALE-01`. **PR #220 and PR #222 rejected (closed without merge, nothing in `main`)**: a design proof (main-liveness non-regression, bounded recoverable ownership, adversarial matrix) is required before the next runtime attempt — [PUBLISH-REPAIR-LIVENESS-REJECTED-DESIGNS.md](./PUBLISH-REPAIR-LIVENESS-REJECTED-DESIGNS.md) | [known issue](./KNOWN_ISSUES.md#issue-publish-repair-renewal-after-classify-01) |
