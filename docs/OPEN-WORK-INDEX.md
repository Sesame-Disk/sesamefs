...
| `ISSUE-PCD1-CERTIFIED-BASELINE-IMPLEMENTATION-01` | HIGH (P1) — OPEN, PC-D1A foundation landed 2026-09-19 | PC-D1A adds the two canonical `libraries` witness columns, `LibraryState` fail-closed validity, HEAD-fenced baseline CAS, and atomic `(H,H,V) -> (H',H',V)` global-SERIAL authority primitives. Remaining PC-D1B work is the exact-P tree walk, non-expiring liveness handshake, cold-path mapping promotion for SHA-1-only identities, and first-use integration; historical backfill is a greenfield non-goal, and no productive consumer or PC-2 work has started | [known issue](./KNOWN_ISSUES.md#issue-pcd1-certified-baseline-implementation-01) · [PC-D1](./PC-D1-INHERITED-DEPENDENCY-CONTINUITY.md) |
