...
- Mutation: `scripts/w2-sync-putblock-head-mutation-validation.sh` M12 bypasses the fallback (RED), M13 removes the fan-out cancellation (RED against the bounded fail-fast test), M14 weakens the named consistency constant to `LOCAL_QUORUM` (RED against the pin test) -- 14/14 mutations produce the expected RED.
...
This is the corrected measurement, using **disjoint, non-overlapping block sets per N** and an explicit `BlockReferenceExistsLocalQuorum` precondition check immediately before timing each scenario, confirming every block in it is still genuinely locally-blind. An earlier version of this test reused overlapping slices (N=10 as blocks[0:10], N=100 as blocks[0:100], etc.), which let an earlier scenario's `EACH_QUORUM` query trigger Cassandra's own background read-repair on a shared block, silently healing dc-na's copy before a later, larger-N scenario queried the same block again -- that version measured 3.4-5.0s at N=1000 across two runs, versus 1.67s with disjoint sets; the corrected number above is authoritative. This environment is container-to-container on one Docker host, not true production WAN, and still confirms the single-DC table's own caveat: real inter-datacenter distance is roughly **15-20x** the single-DC numbers at N=1000 (98.6ms single-DC all-unprovenanced vs. 1.67s real cross-DC all-hit), scaling with `⌈N/20⌉` waves of real round trips.
...
**Decision confirmed 2026-09-09 by the project owner** (not unilaterally by the implementing agent, per explicit review): ship #210 as scoped, accepting the availability-domain change and the real latency cost documented above. The rationale: no cost-free alternative exists within this issue's scope (a plain `QUORUM` fallback is unsafe with one replica per DC; strengthening the `up:` write itself would impose an unconditional cost on every `PutBlock`, not only the ambiguous case), and the fan-out fix bounds the blast radius of both the availability dependency and the latency cost to roughly one concurrency wave per commit rather than unbounded growth.

The case for shipping anyway: (1) this closes a genuine correctness gap -- the alternative is silently under-protecting cross-DC PutBlock provenance, which #206 already established as unacceptable; (2) the cost is bounded and predictable (no unbounded/pathological multiplication, thanks to the fan-out fix), scaling linearly with `⌈N/20⌉` real round trips, not worse; (3) it only manifests for large (roughly 100+) block deltas in a single commit, not the common small-commit case (the N=1/10 rows stay well under 100ms even at real cross-DC distance); and (4) the lever that directly cuts this cost -- raising `syncCommitBlockPlacementConcurrency` above 20 -- is a one-line, low-risk change, but changing it now would mean guessing at a number without real production topology to validate it against, which is exactly the mistake `ISSUE-SYNC-PUTBLOCK-READINESS-HOTPATH-COST-01` exists to avoid.
