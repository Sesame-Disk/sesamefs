...
**Bounded fail-fast**: `syncCommitProvenancedBlockIDs` (the fan-out that calls the scope gate once per distinct block) uses `errgroup.WithContext` instead of a plain `errgroup.Group`; each goroutine checks `ctx.Done()` before issuing its own check, so the first fatal error (local or global) stops issuing additional provenance DB probes (the admission loop keeps creating goroutines for later blocks, but each returns immediately via the cancelled context instead of making its own external call). A degraded/unreachable datacenter therefore fails a large commit's readiness after at most `syncCommitBlockPlacementConcurrency` (20) provenance DB probes in flight, not after attempting the same failing lookup for every one of up to `N` blocks.
...
**What was done instead**: the fan-out that calls the scope gate is now bounded fail-fast (see above), so the *blast radius* of this dependency during a real datacenter outage is capped at at most `syncCommitBlockPlacementConcurrency` (20) lookups per commit, not up to `N`. The trade-off itself -- dedup-only commits now share the cross-DC fallback's availability domain -- is accepted and documented here as a deliberate decision, not silently absorbed into a latency table. It is the direct, unavoidable consequence of closing this P1 using only the signal already available (a referrer-scoped existence check) without a wider redesign.
