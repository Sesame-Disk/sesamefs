...
**Bounded fail-fast**: `syncCommitProvenancedBlockIDs` (the fan-out that calls the scope gate once per distinct block) uses `errgroup.WithContext` instead of a plain `errgroup.Group`; each goroutine checks `ctx.Done()` before issuing its own check, so the first fatal error (local or global) stops scheduling new work. A degraded/unreachable datacenter therefore fails a large commit's readiness after roughly one wave of `syncCommitBlockPlacementConcurrency` (20) in-flight lookups, not after attempting the same failing lookup for every one of up to `N` blocks.
...
**Real cross-DC measurement (the scenario the single-DC table above structurally cannot produce)**: `TestW2SyncXDCAllCrossDCHitCostAtN3DC`, run against the real 3-DC fixture (`scripts/w2-sync-putblock-xdc-provenance-validation.sh`), measured all-cross-DC-hit -- every block genuinely recovered via the `EACH_QUORUM` fallback from `dc-na`, none a local hit or genuinely absent -- at the same concurrency bound (20):

| N | all-cross-DC-hit (real 3-DC, run 1) | run 2 |
|---|---|---|
| 1 | 36.5ms | 28.8ms |
| 10 | 84.2ms | 85.3ms |
| 100 | 874.0ms | 461.5ms |
...
Two independent runs, shown side by side rather than averaged, because the spread itself is informative: this environment (container-to-container on one Docker host, not true production WAN) shows real run-to-run variance at N>=100, consistent with the single-DC table's own noise caveat. Despite the spread, both runs land in the same order of magnitude and confirm the single-DC table's caveat concretely: real inter-datacenter distance is roughly **35-100x** the single-DC numbers at N=1000 (98.6ms single-DC all-unprovenanced vs. 3.4-5.0s real cross-DC all-hit), scaling with `⌈N/20⌉` waves of real round trips.
