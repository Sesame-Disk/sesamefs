...
- Unit: `internal/api/sync_w2_putblock_xdc_provenance_test.go` pins the routing (local hit/error never calls the fallback; local miss + global hit recovers; local miss + global miss stays unprovenanced; local miss + global error fails closed) and proves bounded fail-fast (`TestSyncCommitProvenancedBlockIDs_GlobalFailureStopsSchedulingAdditionalWork`: 200 blocks, all clean local misses, a failing global fallback -- confirmed 20 calls attempted, not 200; confirmed 200/200 without the `errgroup.WithContext` fix).
...
| N | all-cross-DC-hit, scope-gate only (real 3-DC) |
|---|---|
| 1 | 17.1ms |
| 10 | 38.9ms |
| 100 | 291.6ms |
...
This is the corrected measurement, using **disjoint, non-overlapping block sets per N** and an explicit `BlockReferenceExistsLocalQuorum` precondition check immediately before timing each scenario, confirming every block in it is still genuinely locally-blind. An earlier version of this test reused overlapping slices (N=10 as blocks[0:10], N=100 as blocks[0:100], etc.), which let an earlier scenario's `EACH_QUORUM` query trigger Cassandra's own background read-repair on a shared block, silently healing dc-na's copy before a later, larger-N scenario queried the same block again -- a real precondition violation on its own, independent of what it did to the timing. That version measured 3.4-5.0s at N=1000 across two runs; the disjoint-set version measures 1.67s. **The old methodology is invalidated by the precondition violation alone, and its number is retired rather than explained**: a block healed by read-repair would skip the `EACH_QUORUM` round trip entirely on a later scenario, which should have made the contaminated run *faster*, not slower, than the clean one -- the reverse of what was actually measured. What produced the 3.4-5.0s figure (run-to-run variance, host/scheduler load, warmup, or something else) is not established and is not claimed here; only the disjoint-set number (1.67s) is asserted as valid, and it reflects a single recorded run -- treat it as one clean sample, not a tight bound, until a few more runs are captured. This environment is container-to-container on one Docker host, not true production WAN: it confirms real Cassandra multi-DC consistency semantics (`EACH_QUORUM` genuinely crossing per-DC replica sets), not real geographic latency. What it shows is roughly **15-20x** the single-DC numbers at N=1000 (98.6ms single-DC all-unprovenanced vs. 1.67s on this 3-DC fixture), scaling with `⌈N/20⌉` waves of round trips -- a multiplier this fixture demonstrates, not a production WAN latency prediction.
...
Ship the fix as scoped, with the real-latency number taken seriously rather than minimized: correctness is unconditional (the fallback is the only way to close this P1), the query-count formula is exact and bounded (`1` extra `EACH_QUORUM` read per local miss, never on a hit, and the fan-out fix above bounds the failure case to one concurrency wave), but the real 3-DC measurement shows this is not free at scale -- roughly 1.7 seconds at N=1000 (scope-gate only; see the pipeline-cost note above for what a genuine recovery additionally pays) in even a modest container-to-container 3-DC fixture, for *either* an all-recovery or an all-genuinely-unprovenanced large commit. That is a real, user-visible latency cost for large dedup-heavy or cross-DC-recovered commits, not a rounding error.
