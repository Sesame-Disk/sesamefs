...
**Correction to a claim this section originally made**: the all-genuinely-unprovenanced (dedup) case does **not** resolve at the cheap single-DC-table speed in a real multi-DC deployment. `EACH_QUORUM` must contact a quorum of replicas in *every* datacenter to determine "not found" just as much as it does to determine "found" -- absence is not cheaper to prove than presence at this consistency level. In a real production topology, an all-dedup commit pays essentially the same per-wave cross-DC round-trip cost as the all-cross-DC-hit measurement above, not the single-DC dev-stack numbers. The single-DC table's only valid use is measuring *query count* and confirming there is no pathological multiplication of round trips beyond `⌈N/20⌉` waves; for real latency, the row above is the right proxy for both the genuinely-unprovenanced and the genuine-recovery case alike.
