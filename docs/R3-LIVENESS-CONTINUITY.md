...
| Repair after HEAD is visible | Existing `pub:` plus durable pending publication record, when present; a live repair visit renews `pub:<repo:commit:fsID>:<producer_token>` **before** the bounded classifier, and compensation/fence/retention target only that producer-specific referrer. A requeued producer gets a distinct token, so equal or inverted leases cannot cross-fence it; CONSUMED witnesses complete their own 35-day lifecycle independently. Pending identities retain every finished witness; a durable monotonic producer reservation (migration 027) caps new witnesses at 32 per live generation and fails closed before token/intent creation. Destructive pending compaction remains disabled until full fan-out coverage is durable. Discovery after expiry and sequential per-block fan-out expiry remain zero-ref gaps (`ISSUE-PUBLISH-REPAIR-DISCOVERY-SCALE-01`, `ISSUE-GC-PUB-REF-ZERO-REF-01`) | Not a pre-HEAD materialization decision | The shared repair classifier promotes permanent `fs:` only after bounded positive evidence from a durable SERIAL HEAD anchor (`EACH_QUORUM` ancestry, 1024/chunk/30s, resumable cursor of the next unread commit); inconclusive results retain `pub:` and the repair; missing rows are no-ops | R31-C1 closes moving-HEAD convergence and the producer cross-fence cleanup race for this worker. It does not close gap-free handoff, known-loser durability, discovery scale, zero-ref projection, or progress Paxos contention | pending published files and Sync repair paths; see [ISSUE-PUBLISH-REPAIR-DISCOVERY-SCALE-01](KNOWN_ISSUES.md#issue-publish-repair-discovery-scale-01), [ISSUE-GC-PUB-REF-ZERO-REF-01](KNOWN_ISSUES.md#issue-gc-pub-ref-zero-ref-01), [ISSUE-PUBLISH-REPAIR-PROGRESS-PAXOS-DOMAIN-01](KNOWN_ISSUES.md#issue-publish-repair-progress-paxos-domain-01) | `CONDITIONAL`; R31-A/C1 classifier+convergence closed for this worker, overall R31 open |
