...
| Repair after HEAD is visible | Existing `pub:` plus durable pending publication record, when present; a visit that finds a live row writes a transient, write-only walk pin `pub:<repo:commit:fsID>:walk` (1h TTL, distinct referrer, never removed) **before** the bounded classifier under an enforced fan-out deadline, binds the classifier and its progress LWTs to the walk pins' deadline, and then follows `main`-compatible outcome ordering (`ISSUE-PUBLISH-REPAIR-RENEWAL-AFTER-CLASSIFY-01`, closed 2026-09-17) so every walk pin is valid when the post-walk handoff begins — the handoff itself is `main`'s unbounded fan-out (per-block guarantee conditional on its slack), and discovery after the prior pin expired, and expiry during the walk-pin fan-out for blocks not yet pinned, remain zero-ref gaps (`ISSUE-PUBLISH-REPAIR-DISCOVERY-SCALE-01`); the worker best-effort removes that identity (concurrent renewal may leave TTL-bounded `pub:`; `ISSUE-PUBLISH-REPAIR-OWNED-PUB-CLEANUP-RACE-01`) | Not a pre-HEAD materialization decision | The shared repair classifier promotes permanent `fs:` only after bounded positive evidence from a durable SERIAL HEAD anchor (`EACH_QUORUM` ancestry, 1024/chunk/30s, resumable cursor of the next unread commit; clean genesis is persisted as exhausted before re-observing HEAD and may re-anchor with a second 1024-node chunk in the same context); every inconclusive or unsupported-negative result retains `pub:` and the repair; a missing repair row is a no-op | R31-C1 closes moving-HEAD non-convergence, pre-HEAD anchor poisoning, and partial-progress loss under the 30s/EQ budget for this worker. It does not close gap-free `pub:` handoff, known-loser durability, discovery scale, `pub:` zero-ref projection, or per-repair Paxos isolation (`ISSUE-PUBLISH-REPAIR-PROGRESS-PAXOS-DOMAIN-01`) | pending published files and Sync repair paths; see [ISSUE-PUBLISH-REPAIR-TIMEOUT-CLEANUP-01](KNOWN_ISSUES.md#issue-publish-repair-timeout-cleanup-01), [ISSUE-PUBLISH-REPAIR-REACHABILITY-01](KNOWN_ISSUES.md#issue-publish-repair-reachability-01), [ISSUE-PUBLISH-REPAIR-REACHABILITY-CONVERGENCE-01](KNOWN_ISSUES.md#issue-publish-repair-reachability-convergence-01), [ISSUE-PUBLISH-REPAIR-OWNED-PUB-CLEANUP-RACE-01](KNOWN_ISSUES.md#issue-publish-repair-owned-pub-cleanup-race-01), [ISSUE-PUBLISH-REPAIR-RENEWAL-AFTER-CLASSIFY-01](KNOWN_ISSUES.md#issue-publish-repair-renewal-after-classify-01), [ISSUE-PUBLISH-REPAIR-PROGRESS-PAXOS-DOMAIN-01](KNOWN_ISSUES.md#issue-publish-repair-progress-paxos-domain-01) | `CONDITIONAL`; R31-A/C1 classifier+convergence closed for this worker, overall R31 open |
