...
| Repair after HEAD is visible | Existing `pub:` plus durable pending publication record, when present; unresolved visits renew repair-owned `pub:<commitID>` for staged IDs; successful settlement best-effort removes that identity (concurrent renewal may leave TTL-bounded `pub:`; `ISSUE-PUBLISH-REPAIR-OWNED-PUB-CLEANUP-RACE-01`) | Not a pre-HEAD materialization decision | The shared repair classifier promotes permanent `fs:` only after bounded positive evidence from a durable SERIAL HEAD anchor (`EACH_QUORUM` ancestry, 1024/30s, resumable cursor of the next unread commit; clean genesis is persisted as exhausted before re-observing HEAD and may re-anchor to a later SERIAL HEAD); every inconclusive or unsupported-negative result retains `pub:` and the repair; a missing repair row is a no-op | R31-C1 closes moving-HEAD non-convergence, pre-HEAD anchor poisoning, and partial-progress loss under the 30s/EQ budget for this worker. It does not close known-loser durability, discovery scale, or `pub:` zero-ref projection | pending published files and Sync repair paths; see [ISSUE-PUBLISH-REPAIR-TIMEOUT-CLEANUP-01](KNOWN_ISSUES.md#issue-publish-repair-timeout-cleanup-01), [ISSUE-PUBLISH-REPAIR-REACHABILITY-01](KNOWN_ISSUES.md#issue-publish-repair-reachability-01), [ISSUE-PUBLISH-REPAIR-REACHABILITY-CONVERGENCE-01](KNOWN_ISSUES.md#issue-publish-repair-reachability-convergence-01), [ISSUE-PUBLISH-REPAIR-OWNED-PUB-CLEANUP-RACE-01](KNOWN_ISSUES.md#issue-publish-repair-owned-pub-cleanup-race-01) | `CONDITIONAL`; R31-A/C1 classifier+convergence closed for this worker, overall R31 open |
