...
Ten deliberate mutations remain load-bearing: removing or reordering liveness/fence checks, widening provenance scope, weakening fail-closed outcomes, clearing a shared repair row on ambiguous queue failure, skipping successful settlement, clearing on failed finalize, and leaking canonical IDs across files all produce the expected RED.
