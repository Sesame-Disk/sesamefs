...
Branch `fix/w2-sync-putblock-head-continuity`, from `main` at `57fd090d2e98013dd875a4d5fe6c4b95c8df5f08` (the #205 merge). Closes, at unit level, the two `CONDITIONAL` Sync rows of `docs/R3-LIVENESS-CONTINUITY.md`: "Sync `PutBlock` followed by HEAD" and "Sync retry from another pod within the same provisional TTL". Before this, a block's only liveness after `PutBlock` was a fixed 48h `up:sync:<repo>:<sha256>` reference that HEAD never renewed, verified, or re-checked against physical placement.

Reused `published_block_reference_repairs` (PR #205's schema, no new migration) and its exported `internal/api/v2/publish_repair.go` helpers, already used by `internal/api/seafhttp.go`. Before the HEAD CAS in both `handleSyncHeadPromotion` and `tryAutoMergeSyncHeadPromotion`: added files' block IDs are resolved to canonical form once per commit via a single batch call and split back per file (`resolveSyncCommitAddedFilesCanonical`), so the durable repair row always carries each file's own canonical IDs, never a flattened cross-file aggregate or an external SHA-1 (an audit finding against the first draft of this slice, verified against `internal/api/v2/fs_helpers.go`'s `internalBlockIDs` invariant); a durable repair row is queued per added file; own liveness is renewed (never fabricated) and exact physical placement is fail-closed re-validated at LOCAL_QUORUM, strictly scoped to blocks that already carry a real `up:sync:<repo>:<block>` reference (`syncBlockHasOwnLivenessProvenanceFn`, gated by `db.BlockReferenceExists`) — a second audit finding against the first draft, which had applied the same check to every added block regardless of provenance, silently widening scope into the separate "Sync commit whose block had no associated PutBlock" `UNKNOWN` row this slice does not touch.

Post-HEAD, `repairPublishedSyncCommitBlockDelta` only renews liveness best-effort (never fail-closed, since an already-reachable commit cannot be rejected retroactively) and settles the repair row through a shared `finalizeSyncCommitBlockDeltaAndSettleRepairIntent` wrapper (clear on success, schedule for retry on failure) kept outside the frozen `finalizeSyncCommitBlockDelta` root so the `internal/db/r3_publication_hot_path_failclosed_test.go` root list needs no change. A genuinely divergent CAS loss (current HEAD moved to neither the target nor the expected parent) synchronously clears its own repair-row entry; an idempotent same-target "loss" to another writer publishing the identical commit never does, since that writer's own crash recovery may still depend on it — a third audit finding, since the shared `commit_id` identity of Sync's direct-HEAD retries (unlike #205's per-attempt-unique identity) makes an unconditional clear-on-loss unsafe. `tryAutoMergeSyncHeadPromotion`'s `mergedCommitID` is always attempt-unique, so its defer-guarded cleanup can unconditionally clear on any non-ambiguous failure.

All new reads/writes are LOCAL_QUORUM (`ProbeBlockReuse`, `ValidateBorrowedFSPublicationAuthority`, `AddProvisionalBlockReferenceWithExpiry`, `BlockReferenceExists`) — never SERIAL/EACH_QUORUM. The existing `internal/db/r3_publication_shape_contract_test.go` and `r3_publication_hot_path_typed_test.go` frozen baselines needed **no update**: both are static-AST analyses of direct `h.db.<method>()` call shapes inline in the named root functions' own bodies, and the new logic is delegated through separate helper methods/functions (matching how `internal/api/seafhttp.go` already delegates to the same `v2.QueuePublishedFSObjectBlockReferenceRepair`-family helpers), not inlined at the call sites the guards inspect. Confirmed by running the full `internal/db` suite unmodified.

New unit coverage: `internal/api/sync_w2_putblock_head_test.go` (ordering — liveness renewed before fence validated; scope gate — only provenanced blocks reach placement/fence checks; fail-closed — any non-`Reusable`/non-`Authorized` outcome rejects; settlement — clear-on-success, schedule-on-failure, never clear on a failed finalize; identity — per-file canonical split, rollback of earlier queued rows on a later file's queue failure). `internal/api/sync_publish_handshake_test.go`'s existing handshake tests were updated to neutralize the new repair-row/provenance seams (not weakened — the property they test, pub: staged before finalize, is unchanged and still real-entry-point-driven).

Real-Cassandra/MinIO integration evidence (`internal/integration/sync_w2_putblock_head_integration_test.go`, gate `SESAMEFS_REQUIRE_W2_SYNC_PUTBLOCK_HEAD_EVIDENCE=1`, wired into `TestMain`'s OR-chain and `go-integration-test`/`go-all-test`) covers five named legs against the real running stack: `normalFlowRenewsLivenessAndSettles` (forces the `up:` reference's Cassandra-native TTL to near-immediate expiry, proves HEAD renews it to ~48h, permanent `fs:` ref registered, repair row cleared), `crossNodeIdempotentRepairSettlesWithoutProcessMemory` (simulates a crash between HEAD CAS and finalize by stripping the permanent reference, then replays the idempotent retry from a *different* node so the first node's in-process finalize memo cannot mask the repair path), `activeGCClaimBlocksHeadFailClosed` (a real `gc.CassandraStore` delete claim on the block's exact placement makes HEAD fail closed with 503, HEAD unchanged), `definitiveCASLoserCleansOnlyOwnAttempt` (two genuinely conflicting same-path commits race for HEAD; the loser gains no permanent reference and no lingering repair row), and `autoMergeProductionPathSettlesWithRealBlocks` (the existing non-overlapping-entries auto-merge shape driven with real PutBlock-provenanced blocks instead of synthetic empty fs_objects). All five passed on a first-or-second attempt against real Cassandra 5.0/MinIO; the full canonical `go-integration-test` suite (P2/P3/P4A/P4B/R26/X1/BorrowedFS/SessionUpload/W2-post-head/W2-Sync, ~150 tests) is green with zero regressions.

Ten deliberate mutations (`scripts/w2-sync-putblock-head-mutation-validation.sh`, mirroring `scripts/w2-post-head-mutation-validation.sh`'s backup/restore structure) all produced the expected RED against the new unit suite: removing the own-liveness barrier, moving liveness after exact-placement validation, removing the final exact-P validation, inverting the own-liveness provenance scope gate, weakening the placement fail-closed check, weakening the exact-placement fence to accept any outcome, skipping repair-row queue rollback, never clearing the repair row on success, performing repair-row cleanup on a failed/unknown finalize outcome, and leaking canonical block IDs across files in the per-file split.

`docs/KNOWN_ISSUES.md` extends `ISSUE-PUBLISH-REPAIR-REACHABILITY-01` to name the Sync direct-HEAD shared-repair-row case explicitly: a row that is never provably reachable nor provably a definitive loser (crash mid-flight, or a same-target race whose outcome this process never learns) is retained forever by the existing repair-sweep classifier, which only ever promotes or retains, never deletes on inference. This is the same accepted tradeoff #205 already carries for its own funnel, not a new regression, and never a data-safety issue — the durable row has no TTL and can accumulate, but holds no block reference open beyond `up:`/`pub:`'s own independent TTLs. This slice does not close broader W2/R31, "Sync commit whose block had no associated PutBlock", `recv-fs-before-put`, G1, or X1. `GC_ENABLED=false`.
