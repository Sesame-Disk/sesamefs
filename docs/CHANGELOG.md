...
**Cost-benchmark methodology bug**: the real 3-DC all-cross-DC-hit measurement sliced overlapping block ranges per N (N=10 as blocks[0:10], N=100 as blocks[0:100], ...). Cassandra's own background read-repair, triggered when an `EACH_QUORUM` query notices dc-na's replica disagrees with dc-eu's, could silently heal a shared block's local visibility between scenarios -- a real precondition violation on its own. Fixed with disjoint, non-overlapping block sets per N and an explicit `BlockReferenceExistsLocalQuorum` precondition check (fail closed if any block is already locally visible) immediately before timing each scenario. The old (contaminated) run measured 3.4-5.0s at N=1000 across two runs; the corrected, disjoint-set run measures 1.67s. The old methodology is invalidated by the precondition violation alone, and its number is retired rather than explained -- notably, a block healed by read-repair would skip the `EACH_QUORUM` round trip on a later scenario, which should have made the contaminated run *faster*, not slower, the reverse of what was measured. Only the disjoint-set number is asserted as valid, and it is a single recorded run, not a tight bound. `docs/KNOWN_ISSUES.md` also drops "real inter-datacenter distance" language for this container-to-container fixture: it demonstrates real Cassandra multi-DC consistency semantics, not real geographic WAN latency.
