...
**Cost-benchmark methodology bug**: the real 3-DC all-cross-DC-hit measurement sliced overlapping block ranges per N (N=10 as blocks[0:10], N=100 as blocks[0:100], ...). Cassandra's own background read-repair, triggered when an `EACH_QUORUM` query notices dc-na's replica disagrees with dc-eu's, could silently heal a shared block's local visibility between scenarios -- turning a later, larger-N "cross-DC hit" into an accidental local hit and deflating the measurement. Fixed with disjoint, non-overlapping block sets per N and an explicit `BlockReferenceExistsLocalQuorum` precondition check (fail closed if any block is already locally visible) immediately before timing each scenario. This changed the measured N=1000 real-3-DC number from 3.4-5.0s (contaminated) to 1.67s (corrected, authoritative) -- lower, not higher, since the contamination had been letting some blocks resolve via the cheap local path partway through.

**Cost-test cleanup was best-effort, not fail-closed**: a cleanup failure only logged (`t.Logf`) and left the test green, which is exactly wrong for the deliberately non-TTL `gc_provisional_block_refs_by_day` discovery projection -- a silent cleanup failure there is durable residue by construction. `cleanupW2SyncXDCCostFixture` now fails the test (`t.Fatalf`) on any of its three deletes failing, is shared by both the single-DC and 3-DC cost tests, and takes an explicit consistency parameter (`LOCAL_QUORUM` for the single-DC test, `EACH_QUORUM` for the 3-DC one, so the delete is confirmed to have reached every datacenter before the test exits). The 3-DC version also needed the exact `expiresAt` the write leg used (logged as `W2_SYNC_XDC_COST_EXPIRES_AT` and threaded through the script) to reconstruct the by-day projection's clustering key -- a freshly-computed `expiresAt` at cleanup time would not match the row and the delete would silently affect zero rows.
