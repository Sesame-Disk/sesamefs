...
closed 2026-09-12). A visit that finds a live durable repair writes a
transient, write-only walk pin `pub:<repo:commit:fsID>:walk` (1h TTL, a
distinct referrer, stable per row, never removed) **before** the bounded
classifier (`ISSUE-PUBLISH-REPAIR-RENEWAL-AFTER-CLASSIFY-01`, closed
2026-09-17; PR #220's durable-witness protocol was abandoned): the fan-out
runs under an enforced 20 min deadline, a fan-out that fails or overruns
does not start the walk but still takes `main`'s UNKNOWN renewal, the
classifier and its progress LWTs are bound to the walk pins' deadline, and
everything after the classifier is `main`-compatible outcome ordering — the
ordinary 35d renewal of `pub:<repo:commit:fsID>` on UNKNOWN, whose
compensation now decides absence at `EACH_QUORUM`. That closes the
classifier-induced interval: every walk pin is valid when the post-walk
handoff begins. It is still not a globally gap-free handoff: the handoff
is `main`'s unbounded per-block fan-out (the per-block guarantee is
conditional on it landing within the walk pin's remaining slack), if
discovery starts after the prior liveness already expired the gap already
existed, and the walk-pin write is itself a sequential per-block fan-out
during which a prior pin can still expire for blocks not yet pinned
(`ISSUE-PUBLISH-REPAIR-DISCOVERY-SCALE-01`). The shared worker
