...
| `scripts/library-head-serial-domain-mutation-validation.sh` | M1–M4 SERIAL→LOCAL_SERIAL per seam, M5 pin removed, M6 constant = LocalSerial, M7 hidden `IF created_at AND head_commit_id`, M8 hidden `sesamefs.libraries`, M9 `Query(fmt.Sprintf(DELETE IF))`, M10 package-level FuncLit DELETE IF, M11 allowlisted `UpdateLibrary` becomes HEAD LWT, M12 dynamic SET fragment, M13 non-literal lock `fmt.Sprintf`, M14 embedded migration HEAD LWT: all RED |
