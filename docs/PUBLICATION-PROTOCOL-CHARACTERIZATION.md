...
| `ISSUE-LIBRARY-ROLLBACK-GHOST-PROJECTIONS-01` | P2 | FOLLOW-UP (introduced by H1 round 4, accepted as documented debt) | `rollbackNewLibrary` now takes authority with `DELETE libraries ... IF head_commit_id = null` before its cleanup batch; a crash or batch failure between the two leaves active projections, `libraries_by_id`, policies, fs_objects and commits behind a deleted canonical row, and no reconciler covers that state. Needs a durable recovery seam + fault-injection test (`docs/KNOWN_ISSUES.md`). |
