...
2026-09-17; PR #220's durable-witness protocol was abandoned): a walk pin
write error does not start the walk, and everything after the classifier is
`main`'s flow — the ordinary 35d renewal of `pub:<repo:commit:fsID>` on
UNKNOWN, whose compensation now decides absence at `EACH_QUORUM`. That closes
the classifier-induced interval once the walk-pin fan-out has completed; it
is still not a globally gap-free handoff: if discovery starts after the prior
liveness already expired the gap already existed, and the walk-pin write is
itself a sequential per-block fan-out during which a prior pin can still
expire for blocks not yet pinned (`ISSUE-PUBLISH-REPAIR-DISCOVERY-SCALE-01`). The shared worker
