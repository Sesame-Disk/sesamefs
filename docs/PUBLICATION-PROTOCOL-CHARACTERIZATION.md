...
per-row `pub:<repo:commit:fsID>` for the staged block IDs **before** the
...
a row gone before/during the write is a no-op with exact compensation. That
closes the interval a visit could create for itself; it is still not a
globally gap-free handoff: if discovery starts after the prior liveness
already expired, the gap already existed. The shared worker
