...
certified first. PC-1 remains behavior-preserving, with zero funnels migrated; PC-D1A adds authority-only witness state and HEAD CAS primitives. PR #228 implements the PC-D1B.1 read-only identity-authority gate and complete-tree certifier. PC-D1B.3 mapping coverage is implemented in PR #233 (pending merge); lifecycle serialization and productive-consumer integration remain separate PC-D1B work. GC remains a separate PRE-GC safety owner.
